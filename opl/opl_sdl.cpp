...
static void
  FillBuffer(uint8_t *buffer, unsigned int nsamples)
{
...
static void
  OPL_Mix_Callback(void *udata, Uint8 *buffer, int len)
{
...
static int
  OPL_SDL_Init(unsigned int port_base)
{
...
static unsigned int
  OPL_SDL_PortRead(opl_port_t port)
{
...
static void
  OPLTimer_CalculateEndTime(opl_timer_t *timer)
{
  int tics;
...
static void
  WriteRegister(unsigned int reg_num, unsigned int value)
{
  switch (reg_num)
  {
    case OPL_REG_TIMER1:
...
static void
  OPL_SDL_PortWrite(opl_port_t port, unsigned int value)
{
...
static void
  OPL_SDL_SetCallback(uint64_t us, opl_callback_t callback, void *data)
{
  SDL_LockMutex(callback_queue_mutex);
...
static void
  OPL_SDL_SetPaused(int paused)
{
  opl_sdl_paused = paused;
}

static void
  OPL_SDL_AdjustCallbacks(float factor)
{
  SDL_LockMutex(callback_queue_mutex);
