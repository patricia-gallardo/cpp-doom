...
void
  SetProgramLocation(const char *path)
{
  char *p;
...
static void
  WriteResponseFile(const char *iwad, const char *args)
{
  FILE *fstream;
...
static void
  DoExec(const char *executable, const char *iwad, const char *args)
{
  char *argv[3];
...
void
  ExecuteProgram(const char *executable, const char *iwad, const char *args)
{
  pid_t childpid;
  char *homedir;
...
static void
  WriteMessage(FILE *script, char *msg)
{
  char *p;
...
void
  OpenTerminalWindow(const char *doomwadpath)
{
  FILE *stream;
...
void
  OpenDocumentation(const char *filename)
{
  NSString *path;
