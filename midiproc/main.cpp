...
//
static boolean
  WriteInt16(CHAR *out, size_t osize, unsigned int in)
{
  if (osize < 2)
  {
    return false;
...
static boolean
  RegisterSong(const char *filename)
{
...
static boolean
  MidiPipe_RegisterSong(buffer_reader_t *reader)
{
...
static boolean
  MidiPipe_UnregisterSong(buffer_reader_t *reader)
{
  UnregisterSong();
  return true;
}
...
boolean
  MidiPipe_SetVolume(buffer_reader_t *reader)
{
  int     vol;
...
boolean
  MidiPipe_PlaySong(buffer_reader_t *reader)
{
  int     loops;
...
//
boolean
  ParseCommand(buffer_reader_t *reader, uint16_t command)
{
  switch (command)
  {
...
//
void
  InitPipes(HANDLE in, HANDLE out)
{
  midi_process_in  = in;
  ...
  main(int argc, char *argv[])
{
  ...
  if (argc < 5)
  {
    MessageBox(NULL, TEXT("This program is tasked with playing Native ") TEXT("MIDI music, and is intended to be launched by ") TEXT(PACKAGE_NAME) TEXT("."), TEXT(PACKAGE_STRING), MB_OK | MB_ICONASTERISK);
