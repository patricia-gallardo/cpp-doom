...
//
int
  Buffer_Data(buffer_t *buf, byte **data)
{
  *data = buf->data;
  return buf->data_len;
}
...
//
boolean
  Buffer_Push(buffer_t *buf, const void *data, int len)
{
...
//
void
  Buffer_Shift(buffer_t *buf, int len)
{
  ptrdiff_t max_shift;
...
//
void
  DeleteReader(buffer_reader_t *reader)
{
  free(reader);
}
...
//
int
  Reader_BytesRead(buffer_reader_t *reader)
{
...
//
boolean
  Reader_ReadInt8(buffer_reader_t *reader, uint8_t *out)
{
  byte *data, *data_end;
...
//
boolean
  Reader_ReadInt16(buffer_reader_t *reader, uint16_t *out)
{
...
//
boolean
  Reader_ReadInt32(buffer_reader_t *reader, uint32_t *out)
{
  byte *data, *data_end, *dp;
  int   len = Buffer_Data(reader->buffer, &data);
...
//
char *
  Reader_ReadString(buffer_reader_t *reader)
{
