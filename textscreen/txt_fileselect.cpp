...
char *
  TXT_SelectFile(const char *window_title, const char **extensions)
{
  return NULL;
}
...
static char *
  GenerateFilterString(const char **extensions)
{
...
static char *
  SelectDirectory(char *window_title)
{
  LPITEMIDLIST pidl;
  BROWSEINFO   bi;
...
static char *
  CreateEscapedString(const char *original)
{
  char       *result;
  const char *in;
  char       *out;
...
static char *
  CreateExtensionsList(const char **extensions)
{
  char        *result, *escaped;
...
static char *
  GenerateSelector(const char *const window_title, const char **extensions)
{
  const char  *chooser;
  char        *ext_list             = NULL;
...
static char *
  GenerateAppleScript(const char *window_title, const char **extensions)
{
...
static unsigned int
  NumExtensions(const char **extensions)
{
...
//
static char *
  ExpandExtension(const char *orig)
{
  int   oldlen, newlen, i;
  char *c, *newext = NULL;
...
static void
  TXT_FileSelectSizeCalc(TXT_UNCAST_ARG(fileselect))
{
...
static void
  TXT_FileSelectDrawer(TXT_UNCAST_ARG(fileselect))
{
  TXT_CAST_ARG(txt_fileselect_t, fileselect);
...
static void
  TXT_FileSelectDestructor(TXT_UNCAST_ARG(fileselect))
{
  TXT_CAST_ARG(txt_fileselect_t, fileselect);
...
static int
  DoSelectFile(txt_fileselect_t *fileselect)
{
  char *path;
...
static int
  TXT_FileSelectKeyPress(TXT_UNCAST_ARG(fileselect), int key)
{
  TXT_CAST_ARG(txt_fileselect_t, fileselect);
...
static void
  TXT_FileSelectMousePress(TXT_UNCAST_ARG(fileselect),
                           int x,
                           int y,
                           int b)
{
...
static void
  TXT_FileSelectFocused(TXT_UNCAST_ARG(fileselect), int focused)
{
  TXT_CAST_ARG(txt_fileselect_t, fileselect);
...
static void
  InputBoxChanged(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(fileselect))
{
  TXT_CAST_ARG(txt_fileselect_t, fileselect);
