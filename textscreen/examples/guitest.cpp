...
void
  ClosePwnBox(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(window))
{
...
void
  PwnBox(TXT_UNCAST_ARG(widget), void *user_data)
{
  txt_window_t        *window;
...
void
  UpdateLabel(TXT_UNCAST_ARG(widget), void *user_data)
{
  char buf[40];
...
void
  CloseWindow(TXT_UNCAST_ARG(button), void *user_data)
{
  TXT_CloseWindow(firstwin);
}

void
  UnicodeWindow(TXT_UNCAST_ARG(widget), void *user_data)
{
