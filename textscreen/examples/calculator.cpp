...
void
  InsertNumber(TXT_UNCAST_ARG(button), TXT_UNCAST_ARG(value))
{
...
void
  AddNumberButton(txt_table_t *table, int value)
{
  char buf[10];
  int *val_copy;
...
void
  Operator(TXT_UNCAST_ARG(button), TXT_UNCAST_ARG(op))
{
...
void
  AddOperatorButton(txt_table_t *table, const char *label, operator_t op)
{
  char        buf[10];
  operator_t *op_copy;
...
void
  Calculate(TXT_UNCAST_ARG(button), void *unused)
{
  switch (operator)
  {
    case OP_PLUS:
