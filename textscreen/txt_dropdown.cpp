...
static int
  ValidSelection(txt_dropdown_list_t *list)
{
...
static int
  SelectorWindowY(txt_dropdown_list_t *list)
{
  int result;
...
static void
  ItemSelected(TXT_UNCAST_ARG(button), TXT_UNCAST_ARG(callback_data))
{
...
static void
  FreeCallbackData(TXT_UNCAST_ARG(list),
...
static int
  SelectorWindowListener(txt_window_t *window, int key, void *user_data)
{
  if (key == KEY_ESCAPE)
  {
...
static int
  SelectorMouseListener(txt_window_t *window, int x, int y, int b, void *unused)
{
  txt_widget_t *win;
...
static void
  OpenSelectorWindow(txt_dropdown_list_t *list)
{
  txt_window_t *window;
  int           i;
...
static int
  DropdownListWidth(txt_dropdown_list_t *list)
{
  int i;
  int result;
...
static void
  TXT_DropdownListSizeCalc(TXT_UNCAST_ARG(list))
{
...
static void
  TXT_DropdownListDrawer(TXT_UNCAST_ARG(list))
{
  TXT_CAST_ARG(txt_dropdown_list_t, list);
  unsigned int i;
  const char  *str;
...
static void
  TXT_DropdownListDestructor(TXT_UNCAST_ARG(list))
{
}

static int
  TXT_DropdownListKeyPress(TXT_UNCAST_ARG(list), int key)
{
  TXT_CAST_ARG(txt_dropdown_list_t, list);
...
static void
  TXT_DropdownListMousePress(TXT_UNCAST_ARG(list),
                             int x,
                             int y,
                             int b)
{
...
txt_dropdown_list_t *
  TXT_NewDropdownList(int *variable, const char **values, int num_values)
{
