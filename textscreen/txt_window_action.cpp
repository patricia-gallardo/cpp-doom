...
static void
  TXT_WindowActionSizeCalc(TXT_UNCAST_ARG(action))
{
...
static void
  TXT_WindowActionDrawer(TXT_UNCAST_ARG(action))
{
  TXT_CAST_ARG(txt_window_action_t, action);
  int  hovering;
  char buf[10];
...
static void
  TXT_WindowActionDestructor(TXT_UNCAST_ARG(action))
{
  TXT_CAST_ARG(txt_window_action_t, action);
...
static int
  TXT_WindowActionKeyPress(TXT_UNCAST_ARG(action), int key)
{
  TXT_CAST_ARG(txt_window_action_t, action);
...
static void
  TXT_WindowActionMousePress(TXT_UNCAST_ARG(action),
                             int x,
                             int y,
                             int b)
{
...
txt_window_action_t *
  TXT_NewWindowAction(int key, const char *label)
{
...
static void
  WindowCloseCallback(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(window))
{
...
static void
  WindowSelectCallback(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(window))
{
  TXT_CAST_ARG(txt_window_t, window);
...
txt_window_action_t *
  TXT_NewWindowEscapeAction(txt_window_t *window)
{
...
txt_window_action_t *
  TXT_NewWindowAbortAction(txt_window_t *window)
{
  txt_window_action_t *action;
...
txt_window_action_t *
  TXT_NewWindowSelectAction(txt_window_t *window)
{
  txt_window_action_t *action;
