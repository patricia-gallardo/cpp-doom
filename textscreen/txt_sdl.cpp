...
static const txt_font_t *
  FontForName(const char *name)
{
  int               i;
  ...
  TXT_Init()
{
  ...
  TXT_SDLWindow =
    SDL_CreateWindow("", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, screen_image_w, screen_image_h, flags);
...
void
  TXT_SetColor(txt_color_t color, std::uint8_t r, std::uint8_t g, std::uint8_t b)
{
...
static int
  LimitToRange(int val, int min, int max)
{
  if (val < min)
  {
    return min;
...
void
  TXT_UpdateScreenArea(int x, int y, int w, int h)
{
...
void
  TXT_GetMousePosition(int *x, int *y)
{
...
static int
  TranslateScancode(SDL_Scancode scancode)
{
  switch (scancode)
  {
...
static int
  TranslateKeysym(const SDL_Keysym *sym)
{
  int translated;
...
static int
  SDLButtonToTXTButton(int button)
{
  switch (button)
  {
...
static int
  SDLWheelToTXTButton(const SDL_MouseWheelEvent *wheel)
{
  if (wheel->y <= 0)
  {
...
int
  TXT_GetModifierState(txt_modifier_t mod)
{
  SDL_Keymod state;
...
int
  TXT_UnicodeCharacter(unsigned int c)
{
  unsigned int i;
...
void
  TXT_GetKeyDescription(int key, char *buf, size_t buf_len)
{
  const char *keyname;
  int         i;
...
void
  TXT_SetInputMode(txt_input_mode_t mode)
{
...
void
  TXT_SetWindowTitle(const char *title)
{
  SDL_SetWindowTitle(TXT_SDLWindow, title);
}

void
  TXT_SDL_SetEventCallback(TxtSDLEventCallbackFunc callback, void *user_data)
{
...
void
  TXT_StringCopy(char *dest, const char *src, size_t dest_len)
{
  if (dest_len < 1)
  {
    return;
...
void
  TXT_StringConcat(char *dest, const char *src, size_t dest_len)
{
  size_t offset;
...
int
  TXT_vsnprintf(char *buf, size_t buf_len, const char *s, va_list args)
{
  int result;
...
int
  TXT_snprintf(char *buf, size_t buf_len, const char *s, ...)
{
  va_list args;
  int     result;
  va_start(args, s);
