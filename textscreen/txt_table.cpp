...
static int
  IsActualWidget(txt_widget_t *widget)
{
  return widget != NULL
...
void
  TXT_ClearTable(TXT_UNCAST_ARG(table))
{
...
static void
  TXT_TableDestructor(TXT_UNCAST_ARG(table))
{
  TXT_CAST_ARG(txt_table_t, table);
...
static void
  CellOverflowedSize(txt_table_t *table, int x, int y, int *w, int *h)
{
  txt_widget_t *widget;
  int           x1, y1;
...
static int
  IsOverflowingCell(txt_table_t *table, int x, int y)
{
  int w, h;
...
static void
  CalculateWidgetDimensions(txt_table_t  *table,
                            int           x,
                            int           y,
...
static void
  CalcRowColSizes(txt_table_t  *table,
                  unsigned int *row_heights,
...
static void
  TXT_CalcTableSize(TXT_UNCAST_ARG(table))
{
  TXT_CAST_ARG(txt_table_t, table);
  unsigned int *column_widths;
  unsigned int *row_heights;
...
static void
  FillRowToEnd(txt_table_t *table)
{
...
void
  TXT_AddWidget(TXT_UNCAST_ARG(table), TXT_UNCAST_ARG(widget))
{
  TXT_CAST_ARG(txt_table_t, table);
...
static int
  SelectableCell(txt_table_t *table, int x, int y)
{
  txt_widget_t *widget;
  int           i;
...
static int
  FindSelectableColumn(txt_table_t *table, int row, int start_col)
{
  int x;
...
static void
  ChangeSelection(txt_table_t *table, int x, int y)
{
...
static int
  TXT_TableKeyPress(TXT_UNCAST_ARG(table), int key)
{
  TXT_CAST_ARG(txt_table_t, table);
  int selected;
  int rows;
...
static void
  CheckValidSelection(txt_table_t *table)
{
  int rows;
  int new_x, new_y;
...
static void
  LayoutCell(txt_table_t *table, int x, int y, int draw_x, int draw_y)
{
  txt_widget_t *widget;
  int           col_width;
...
static void
  TXT_TableLayout(TXT_UNCAST_ARG(table))
{
  TXT_CAST_ARG(txt_table_t, table);
...
static void
  TXT_TableDrawer(TXT_UNCAST_ARG(table))
{
  TXT_CAST_ARG(txt_table_t, table);
  txt_widget_t *widget;
  int           i;
...
static void
  TXT_TableMousePress(TXT_UNCAST_ARG(table), int x, int y, int b)
{
  TXT_CAST_ARG(txt_table_t, table);
  txt_widget_t *widget;
  int           i;
...
static int
  TXT_TableSelectable(TXT_UNCAST_ARG(table))
{
  TXT_CAST_ARG(txt_table_t, table);
  int i;
...
static void
  TXT_TableFocused(TXT_UNCAST_ARG(table), int focused)
{
  TXT_CAST_ARG(txt_table_t, table);
  int i;
...
txt_widget_t *
  TXT_GetSelectedWidget(TXT_UNCAST_ARG(table))
{
  TXT_CAST_ARG(txt_table_t, table);
  txt_widget_t *result;
  int           index;
...
int
  TXT_SelectWidget(TXT_UNCAST_ARG(table), TXT_UNCAST_ARG(widget))
{
  TXT_CAST_ARG(txt_table_t, table);
  TXT_CAST_ARG(txt_widget_t, widget);
  int i;
