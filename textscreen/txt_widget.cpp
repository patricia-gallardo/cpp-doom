...
std::shared_ptr<txt_callback_table_t>
  TXT_NewCallbackTable()
{
...
void
  TXT_InitWidget(TXT_UNCAST_ARG(widget), txt_widget_class_t *widget_class)
{
...
void
  TXT_SignalConnect(TXT_UNCAST_ARG(widget),
...
void
  TXT_EmitSignal(TXT_UNCAST_ARG(widget), const char *signal_name)
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
void
  TXT_CalcWidgetSize(TXT_UNCAST_ARG(widget))
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
void
  TXT_DrawWidget(TXT_UNCAST_ARG(widget))
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
void
  TXT_DestroyWidget(TXT_UNCAST_ARG(widget))
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
int
  TXT_WidgetKeyPress(TXT_UNCAST_ARG(widget), int key)
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
void
  TXT_SetWidgetFocus(TXT_UNCAST_ARG(widget), int focused)
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
void
  TXT_SetWidgetAlign(TXT_UNCAST_ARG(widget), txt_horiz_align_t horiz_align)
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
void
  TXT_WidgetMousePress(TXT_UNCAST_ARG(widget), int x, int y, int b)
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
void
  TXT_LayoutWidget(TXT_UNCAST_ARG(widget))
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
int
  TXT_AlwaysSelectable(TXT_UNCAST_ARG(widget))
{
  return 1;
}

int
  TXT_NeverSelectable(TXT_UNCAST_ARG(widget))
{
  return 0;
}

int
  TXT_SelectableWidget(TXT_UNCAST_ARG(widget))
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
int
  TXT_ContainsWidget(TXT_UNCAST_ARG(haystack), TXT_UNCAST_ARG(needle))
{
...
int
  TXT_HoveringOverWidget(TXT_UNCAST_ARG(widget))
{
  TXT_CAST_ARG(txt_widget_t, widget);
...
void
  TXT_SetWidgetBG(TXT_UNCAST_ARG(widget))
{
  TXT_CAST_ARG(txt_widget_t, widget);
