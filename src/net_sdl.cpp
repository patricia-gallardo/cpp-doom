...
static boolean
  AddressesEqual(IPaddress *a, IPaddress *b)
{
...
static net_addr_t *
  NET_SDL_FindAddress(IPaddress *addr)
{
...
static void
  NET_SDL_FreeAddress(net_addr_t *addr)
{
  int i;
...
static void
  NET_SDL_SendPacket(net_addr_t *addr, net_packet_t *packet)
{
...
static boolean
  NET_SDL_RecvPacket(net_addr_t **addr, net_packet_t **packet)
{
  int result;
...
void
  NET_SDL_AddrToString(net_addr_t *addr, char *buffer, int buffer_len)
{
  IPaddress *ip;
  uint32_t   host;
  uint16_t   port;
  ...
  M_snprintf(buffer, buffer_len, "%i.%i.%i.%i", (host >> 24) & 0xff, (host >> 16) & 0xff, (host >> 8) & 0xff, host & 0xff);
...
net_addr_t *
  NET_SDL_ResolveAddress(const char *address)
{
  IPaddress   ip;
  char       *addr_hostname;
  int         addr_port;
