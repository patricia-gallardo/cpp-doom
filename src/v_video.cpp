...
//
void
  V_CopyRect(int srcx, int srcy, pixel_t *source, int width, int height, int destx, int desty)
{
  pixel_t *src;
  pixel_t *dest;
...
static inline pixel_t
  drawpatchpx00(const pixel_t dest, const pixel_t source)
...
static inline pixel_t
  drawpatchpx01(const pixel_t dest, const pixel_t source)
#ifndef CRISPY_TRUECOLOR
{
...
static inline pixel_t
  drawpatchpx10(const pixel_t dest, const pixel_t source)
#ifndef CRISPY_TRUECOLOR
{
...
static inline pixel_t
  drawpatchpx11(const pixel_t dest, const pixel_t source)
#ifndef CRISPY_TRUECOLOR
{
...
void
  V_DrawPatchFullScreen(patch_t *patch, boolean flipped)
{
...
void
  V_DrawXlaPatch(int x, int y, patch_t *patch)
{
  int       count, col;
  column_t *column;
  pixel_t  *desttop, *dest;
...
void
  V_DrawBlock(int x, int y, int width, int height, pixel_t *src)
{
  pixel_t *dest;
...
void
  V_DrawScaledBlock(int x, int y, int width, int height, pixel_t *src)
{
  pixel_t *dest;
  int      i, j;
...
void
  V_CopyScaledBuffer(pixel_t *dest, pixel_t *src, size_t size)
{
  int i, j;
...
void
  WritePCXfile(char *filename, pixel_t *data, int width, int height, byte *palette)
{
  int    i;
  int    length;
  pcx_t *pcx;
...
static void
  error_fn(png_structp p, png_const_charp s)
{
  printf("libpng error: %s\n", s);
}

static void
  warning_fn(png_structp p, png_const_charp s)
{
  printf("libpng warning: %s\n", s);
}

void
  WritePNGfile(char *filename, pixel_t *data, int width, int height, byte *palette)
{
  png_structp ppng;
  png_infop   pinfo;
  //  png_colorp pcolor;
...
void
  V_ScreenShot(const char *format)
{
  int         i;
  ...
  if (png_screenshots)
  {
    WritePNGfile(lbmname, I_VideoBuffer, SCREENWIDTH, SCREENHEIGHT, cache_lump_name<byte *>(DEH_String("PLAYPAL"), PU_CACHE));
  }
  else
#endif
  {
    // save the pcx file
    WritePCXfile(lbmname, I_VideoBuffer, SCREENWIDTH, SCREENHEIGHT, cache_lump_name<byte *>(DEH_String("PLAYPAL"), PU_CACHE));
  }
}
...
static void
  DrawNonAcceleratingBox(int speed)
{
  int white;
  int linelen;
  ...
  V_DrawMouseSpeedBox(int speed)
{
  ...
  V_DrawFilledBox(MOUSE_SPEED_BOX_X, MOUSE_SPEED_BOX_Y, MOUSE_SPEED_BOX_WIDTH, MOUSE_SPEED_BOX_HEIGHT, bgcolor);
  V_DrawBox(MOUSE_SPEED_BOX_X, MOUSE_SPEED_BOX_Y, MOUSE_SPEED_BOX_WIDTH, MOUSE_SPEED_BOX_HEIGHT, bordercolor);
