...
static drawpatchpx_t *const drawpatchpx_a[2][2] = {
  {drawpatchpx11,  drawpatchpx10},
  { drawpatchpx01, drawpatchpx00}
};
...
void WritePCXfile(char *filename, pixel_t *data, int width, int height, uint8_t *palette) {
...
void WritePNGfile(char *filename, pixel_t *, int width, int height, uint8_t *palette) {
  //  png_colorp pcolor;
  int j = 0;
...
*/
  ...
  png_structp ppng = png_create_write_struct(PNG_LIBPNG_VER_STRING, nullptr, error_fn, warning_fn);
  if (!ppng) {
    fclose(handle);
    return;
  }
...
#ifdef HAVE_LIBPNG
  if (png_screenshots) {
    WritePNGfile(lbmname, g_i_video_globals->I_VideoBuffer, SCREENWIDTH, SCREENHEIGHT, cache_lump_name<uint8_t *>(DEH_String("PLAYPAL"), PU_CACHE));
  } else
#endif
  {
    // save the pcx file
    WritePCXfile(lbmname, g_i_video_globals->I_VideoBuffer, SCREENWIDTH, SCREENHEIGHT, cache_lump_name<uint8_t *>(DEH_String("PLAYPAL"), PU_CACHE));
  }
}
