...
boolean
  P_GiveWeapon(player_t *player, weapontype_t weapon)
{
  boolean gaveAmmo;
  boolean gaveWeapon;
...
void
  P_SetDormantArtifact(mobj_t *arti)
{
...
void
  P_TouchWhirlwind(mobj_t *target)
{
  int randVal;
...
boolean
  P_AutoUseChaosDevice(player_t *player)
{
  int i;
  ...
  P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
  ...
  if (inflictor && (!source || !source->player || source->player->readyweapon != wp_gauntlets)
