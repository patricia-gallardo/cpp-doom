...
void
  P_CloseWeapons();
void
  P_AddMaceSpot(mapthing_t *mthing);
void
  P_RepositionMace(mobj_t *mo);
void
...
void
  P_ActivateBeak(player_t *player);
void
  P_PostChickenWeapon(player_t *player, weapontype_t weapon);
void
  P_UpdateBeak(player_t *player, pspdef_t *psp);
...
void
  P_InitMonsters();
void
  P_AddBossSpot(fixed_t x, fixed_t y, angle_t angle);
void
  P_Massacre();
void
  P_DSparilTeleport(mobj_t *actor);
...
boolean
  P_GiveAmmo(player_t *player, ammotype_t ammo, int count);
boolean
...
boolean
  P_ChickenMorphPlayer(player_t *player);
