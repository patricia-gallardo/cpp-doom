...
static void *
  DEH_FrameStart(deh_context_t *context, char *line)
{
  int      frame_number = 0;
...
static boolean
  GetActionPointerForOffset(int offset, void **result)
{
  int i;
...
static void
  SuggestOtherVersions(unsigned int offset)
{
  unsigned int i, v;
...
static void
  DEH_FrameParseLine(deh_context_t *context, char *line, void *tag)
{
  state_t *state;
...
static void
  DEH_FrameSHA1Sum(sha1_context_t *context)
{
  int i;
