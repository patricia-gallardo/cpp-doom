  ...
  P_FireWeapon(player_t *player)
{
  ...
  attackState = (player->refire ? wpinfo[player->readyweapon].holdatkstate : wpinfo[player->readyweapon].atkstate);
  ...
  A_Raise(player_t *player, pspdef_t *psp)
{
  ...
  if (player->powers[pw_weaponlevel2])
  {
    P_SetPsprite(player, ps_weapon, wpnlev2info[player->readyweapon].readystate);
  }
  else
  {
    P_SetPsprite(player, ps_weapon, wpnlev1info[player->readyweapon].readystate);
...
void
  A_FireMacePL1B(player_t *player, pspdef_t *psp)
{
  mobj_t *pmo;
  mobj_t *ball;
  angle_t angle;
  ...
  // Which simplifies to:
  //   (pmo->flags2 & 1)
  ball         = P_SpawnMobj(pmo->x, pmo->y, pmo->z + 28 * FRACUNIT - FOOTCLIPSIZE * (pmo->flags2 & 1), MT_MACEFX2);
