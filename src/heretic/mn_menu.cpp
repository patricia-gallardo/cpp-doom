...
static void
  InitFonts();
static void
  SetMenu(MenuType_t menu);
static boolean
  SCNetCheck(int option);
static boolean
  SCQuitGame(int option);
static boolean
  SCEpisode(int option);
static boolean
  SCSkill(int option);
static boolean
  SCMouseSensi(int option);
static boolean
  SCSfxVolume(int option);
static boolean
  SCMusicVolume(int option);
static boolean
  SCScreenSize(int option);
static boolean
  SCLoadGame(int option);
static boolean
  SCSaveGame(int option);
static boolean
  SCMessages(int option);
static boolean
  SCEndGame(int option);
static boolean
  SCInfo(int option);
static boolean
  CrispySmoothing(int option);
static boolean
  CrispyAutomapStats(int option);
static boolean
  CrispyLevelTime(int option);
static boolean
  CrispyPlayerCoords(int option);
static void
  DrawMainMenu();
static void
  DrawEpisodeMenu();
...
static void
  DrawSaveMenu();
static void
  DrawSlider(Menu_t *menu, int item, int width, int slot);
static void
  DrawCrispnessMenu();
void
                  MN_LoadSlotText();
...
static Menu_t OptionsMenu = {
  ...
  DrawOptionsMenu,
  5 + 1,
  OptionsItems, // [JN] Extra item: Crispness menu
  0,
  MENU_MAIN
};
...
void
  MN_DrTextA(const char *text, int x, int y)
{
  char     c;
  patch_t *p;
...
void
  MN_DrTextB(const char *text, int x, int y)
{
  char     c;
  patch_t *p;
  ...
  if (MenuActive == false)
  {
    if (askforquit)
    {
      ...
      if (typeofask == 3)
      {
        ...
        MN_DrTextA(DEH_String("?"), 160 + MN_TextAWidth(SlotText[quicksave - 1]) / 2, 90);
      }
      if (typeofask == 4)
      {
        MN_DrTextA(SlotText[quickload - 1], 160 - MN_TextAWidth(SlotText[quickload - 1]) / 2, 90);
        MN_DrTextA(DEH_String("?"), 160 + MN_TextAWidth(SlotText[quickload - 1]) / 2, 90);
      }
  ...
  DrawSlider(Menu_t *menu, int item, int width, int slot)
{
  ...
  {
    V_DrawPatch(x2, y, cache_lump_name<patch_t *>(DEH_String(count & 1 ? "M_SLDMD1" : "M_SLDMD2"), PU_CACHE));
  }
  V_DrawPatch(x2, y, cache_lump_name<patch_t *>(DEH_String("M_SLDRT"), PU_CACHE));
  V_DrawPatch(x + 4 + slot * 8, y + 7, cache_lump_name<patch_t *>(DEH_String("M_SLDKB"), PU_CACHE));
}
