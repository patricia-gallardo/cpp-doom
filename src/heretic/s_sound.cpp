...
void
  S_ShutDown();
boolean
                 S_StopSoundID(int sound_id, int priority);
...
void
  S_StartSound(void *_origin, int sound_id)
{
  ...
  if (i >= snd_Channels)
  {
    ...
    {
      if (AmbChan != -1 && S_sfx[sound_id].priority <= S_sfx[channel[AmbChan].sound_id].priority)
      {
...
void
  S_StartSoundAtVolume(void *_origin, int sound_id, int volume)
{
  mobj_t *origin = static_cast<mobj_t *>(_origin);
  mobj_t *listener;
  int     i;
...
void
  S_SoundLink(mobj_t *oldactor, mobj_t *newactor)
{
  int i;
  ...
  S_SetMaxVolume(boolean fullprocess)
{
  ...
  {
    soundCurve[0] =
      (*((byte *)cache_lump_name<patch_t *>("SNDCURVE", PU_CACHE)) * (snd_MaxVolume * 8)) >> 7;
  }
  else
  {
    ...
    {
      soundCurve[i] =
        (*((byte *)cache_lump_name<patch_t *>("SNDCURVE", PU_CACHE) + i) * (snd_MaxVolume * 8)) >> 7;
    }
  }
}
