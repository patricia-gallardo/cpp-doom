...
static void
  CheatPowerFunc(player_t *player, Cheat_t *cheat);
static void
...
static void
  CheatArtifact1Func(player_t *player, Cheat_t *cheat);
static void
  CheatArtifact2Func(player_t *player, Cheat_t *cheat);
static void
  CheatArtifact3Func(player_t *player, Cheat_t *cheat);
static void
  CheatWarpFunc(player_t *player, Cheat_t *cheat);
static void
  CheatChickenFunc(player_t *player, Cheat_t *cheat);
static void
  CheatMassacreFunc(player_t *player, Cheat_t *cheat);
static void
  CheatIDKFAFunc(player_t *player, Cheat_t *cheat);
static void
                 CheatIDDQDFunc(player_t *player, Cheat_t *cheat);
  ...
  SB_Drawer()
{
  ...
  else
  {
    if (SB_state == -1)
    {
      ...
      {
        V_DrawPatch(16, 167, cache_lump_name<patch_t *>(DEH_String("GOD1"), PU_CACHE));
        V_DrawPatch(287, 167, cache_lump_name<patch_t *>(DEH_String("GOD2"), PU_CACHE));
      }
      oldhealth = -1;
    }
    DrawCommonBar();
  ...
  if (CPlayer->powers[pw_flight])
  {
        ...
        || !(CPlayer->powers[pw_flight] & 16))
    {
      ...
      if (CPlayer->mo->flags2 & MF2_FLY)
      {
        ...
        {
          V_DrawPatch(20, 17, cache_lump_num<patch_t *>(spinflylump + 15, PU_CACHE));
        }
        else
        {
          V_DrawPatch(20, 17, cache_lump_num<patch_t *>(spinflylump + frame, PU_CACHE));
  ...
  if (CPlayer->powers[pw_weaponlevel2] && !CPlayer->chickenTics)
  {
    ...
    {
      frame = (leveltime / 3) & 15;
      V_DrawPatch(300, 17, cache_lump_num<patch_t *>(spinbooklump + frame, PU_CACHE));
      BorderTopRefresh = true;
      UpdateState |= I_MESSAGES;
    }
    else
  ...
  DrawMainBar()
{
           ...
           || oldartiCount != CPlayer->inventory[inv_ptr].count)
  {
    ...
    {
      V_DrawPatch(179, 160, cache_lump_name<patch_t *>(DEH_String(patcharti[CPlayer->readyArtifact]), PU_CACHE));
  ...
  if (oldammo != temp || oldweapon != CPlayer->readyweapon)
  {
    ...
    if (temp && CPlayer->readyweapon > 0 && CPlayer->readyweapon < 7)
    {
      ...
      V_DrawPatch(111, 172, cache_lump_name<patch_t *>(DEH_String(ammopic[CPlayer->readyweapon - 1]), PU_CACHE));
    }
    oldammo   = temp;
  ...
  DrawFullScreenStuff()
{
  ...
  else
  {
    ...
    for (i = 0; i < 7; i++)
    {
      V_DrawTLPatch(50 + i * 31, 168, cache_lump_name<patch_t *>(DEH_String("ARTIBOX"), PU_CACHE));
      if (CPlayer->inventorySlotNum > x + i
          && CPlayer->inventory[x + i].type != arti_none)
      {
        patch = DEH_String(patcharti[CPlayer->inventory[x + i].type]);
        V_DrawPatch(50 + i * 31, 168, cache_lump_name<patch_t *>(patch, PU_CACHE));
