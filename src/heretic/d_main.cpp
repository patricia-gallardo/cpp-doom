...
char tmsg[300];
void
  tprintf(const char *msg, int initflag)
{
  printf("%s", msg);
}
  ...
  D_DoomMain()
{
  ...
  {
    char temp[64];
    DEH_snprintf(temp, sizeof(temp), "Warp to Episode %d, Map %d, Skill %d ", startepisode, startmap, startskill + 1);
    status(temp);
  }
