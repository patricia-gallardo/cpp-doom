...
void
  I_SetGrabMouseCallback(grabmouse_callback_t func)
{
...
void
  I_DisplayFPSDots(boolean dots_on)
{
  display_fps_dots = dots_on;
}

static void
  SetShowCursor(bool show)
{
  if (!screensaver_mode)
  {
...
static boolean
  ToggleFullScreenKeyShortcut(SDL_Keysym *sym)
{
...
static void
  LimitTextureSize(int *w_upscale, int *h_upscale)
{
...
static void
  CreateUpscaledTexture(boolean force)
{
  int          w, h;
...
void
  I_SetWindowTitle(const char *title)
{
  window_title = title;
}
  ...
  I_InitWindowIcon()
{
  ...
  surface = SDL_CreateRGBSurfaceFrom((void *)icon_data, icon_w, icon_h, 32, icon_w * 4, 0xff << 24, 0xff << 16, 0xff << 8, 0xff << 0);
...
// display.
static void
  CenterWindow(int *x, int *y, int w, int h)
{
  SDL_Rect bounds;
...
void
  I_GetWindowPosition(int *x, int *y, int w, int h)
{
...
void
  I_RenderReadPixels(byte **data, int *w, int *h, int *p)
{
  SDL_Rect         rect;
...
const pixel_t
  I_MapRGB(const uint8_t r, const uint8_t g, const uint8_t b)
{
  /*
      return amask |
              (((r * rmask) >> 8) & rmask) |
