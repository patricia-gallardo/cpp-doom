...
static void
  NET_Query_SendMasterQuery(net_addr_t *addr)
{
  net_packet_t *packet;
...
static query_target_t *
  GetTargetForAddr(net_addr_t *addr, boolean create)
{
...
static void
  NET_Query_SendQuery(net_addr_t *addr)
{
...
static void
  NET_Query_ParseResponse(net_addr_t *addr, net_packet_t *packet, net_query_callback_t callback, void *user_data)
{
...
static void
  NET_Query_ParseMasterResponse(net_addr_t   *master_addr,
                                net_packet_t *packet)
{
  unsigned int    packet_type;
  query_target_t *target;
...
static void
  NET_Query_ParsePacket(net_addr_t *addr, net_packet_t *packet, net_query_callback_t callback, void *user_data)
{
  query_target_t *target;
...
static void
  NET_Query_GetResponse(net_query_callback_t callback,
                        void                *user_data)
{
  net_addr_t   *addr;
  net_packet_t *packet;
...
static void
  NET_Query_QueryLoop(net_query_callback_t callback, void *user_data)
{
...
static void
  NET_Query_ExitCallback(net_addr_t *addr, net_querydata_t *data, unsigned int ping_time, void *user_data)
{
  NET_Query_ExitLoop();
}
...
static void
  formatted_printf(int wide, const char *s, ...) PRINTF_ATTR(2, 3);
static void
  formatted_printf(int wide, const char *s, ...)
{
  va_list args;
  int     i;
...
static constexpr const char *
  GameDescription(int mode, int mission)
{
  switch (mission)
  {
    case doom:
...
static void
  NET_QueryPrintCallback(net_addr_t      *addr,
                         net_querydata_t *data,
                         unsigned int     ping_time,
                         void            *user_data)
{
...
void
  NET_QueryAddress(char *addr_str)
{
  net_addr_t     *addr;
  query_target_t *target;
...
char *
  NET_EndSecureDemo(sha1_digest_t demo_hash)
{
  net_packet_t *request, *response;
  net_addr_t   *master_addr;
  char         *signature;
