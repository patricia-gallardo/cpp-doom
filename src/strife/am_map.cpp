    ...
    */
    else if (key == key_map_mark)
    {
      ...
      if (markpointnum == AM_NUMMARKPOINTS)
        --markpointnum;
      M_snprintf(buffer, sizeof(buffer), "%s %d", DEH_String(AMSTR_MARKEDSPOT), markpointnum + 1); // [STRIFE]
      plr->message = buffer;
      AM_addMark();
    }
  ...
  AM_drawPlayers()
{
  ...
  if (!netgame)
  {
    ...
    AM_drawLineCharacter(player_arrow, arrlen(player_arrow), 0, plr->mo->angle, 224, plr->mo->x, plr->mo->y);
    return;
  }
  ...
  AM_drawThings()
{
  ...
  for (i = 0; i < numsectors; i++)
  {
    ...
    while (t)
    {
      ...
      AM_drawLineCharacter(thintriangle_guy, arrlen(thintriangle_guy), radius, t->angle, colors, t->x, t->y);
