...
//
void
  A_FireFlameThrower(player_t *player, pspdef_t *psp)
{
  mobj_t *mo;
  int     t;
...
//
void
  A_FireMauler2(player_t *player, pspdef_t *pspr)
{
  P_SetMobjState(player->mo, S_PLAY_06);
...
//
void
  A_FireGrenade(player_t *player, pspdef_t *pspr)
{
  mobjtype_t type;
  mobj_t    *mo;
  state_t   *st1;
...
void
  A_FireElectricBolt(player_t *player, pspdef_t *pspr)
{
...
void
  A_FirePoisonBolt(player_t *player, pspdef_t *pspr)
{
  angle_t an = player->mo->angle;
  int     t;
...
//
void
  A_FireRifle(player_t *player, pspdef_t *pspr)
{
...
//
void
  A_FireMauler1(player_t *player, pspdef_t *pspr)
{
  int     i;
  angle_t angle;
  int     damage;
  ...
  if (player->ammo[weaponinfo[player->readyweapon].ammo] >= 20)
  {
    ...
    for (i = 0; i < 20; i++)
    {
      ...
      angle += (t - P_Random()) << 19;
      t = P_Random();
      P_LineAttack(player->mo, angle, 2112 * FRACUNIT, bulletslope + ((t - P_Random()) << 5), damage);
    }
  }
}
...
//
void
  A_SigilSound(player_t *player, pspdef_t *pspr)
{
...
//
void
  A_FireSigil(player_t *player, pspdef_t *pspr)
{
  mobj_t *mo;
  angle_t an;
  int     i;
...
//
void
  A_GunFlashThinker(player_t *player, pspdef_t *pspr)
{
...
//
void
  A_SigilShock(player_t *player, pspdef_t *psp)
{
  player->extralight = -3;
}
...
//
void
  A_MaulerSound(player_t *player, pspdef_t *psp)
{
  int t;
