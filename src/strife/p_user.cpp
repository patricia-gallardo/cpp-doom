...
void
  P_DropInventoryItem(player_t *player, int sprite); // villsa [STRIFE]
boolean
            P_ItemBehavior(player_t *player, int item); // villsa [STRIFE]
    ...
    if (player->weaponowned[newweapon] && newweapon != player->readyweapon)
    {
      ...
      if (weaponinfo[newweapon].availabledemo || !isdemoversion)
      {
        ...
        else
        {
          ...
          if (newweapon == wp_elecbow && player->ammo[am_poisonbolts] && player->readyweapon != wp_poisonbow)
          {
            player->pendingweapon = wp_poisonbow;
          }
          // decide between hp grenade launcher or wp grenade launcher
          else if (newweapon == wp_hegrenade && player->ammo[am_wpgrenades] && player->readyweapon != wp_wpgrenade)
          {
...
//
const char *
  P_RemoveInventoryItem(player_t *player, int slot, int amount)
{
  mobjtype_t type;
...
//
void
  P_DropInventoryItem(player_t *player, int sprite)
{
  int          invslot;
  inventory_t *item;
  mobjtype_t   type;
...
//
boolean
  P_TossDegninOre(player_t *player)
{
  angle_t angle;
  mobj_t *mo;
  mobj_t *ore;
...
//
boolean
  P_SpawnTeleportBeacon(player_t *player)
{
  angle_t angle;
  int     r;
  mobj_t *mo;
...
//
boolean
  P_UseInventoryItem(player_t *player, int item)
{
  int         i;
  const char *name;
  ...
  for (i = 0; i < player->numinventory; i++)
  {
    ...
    M_snprintf(useinventorymsg, sizeof(useinventorymsg), "You used the %s.", name);
...
//
boolean
  P_ItemBehavior(player_t *player, int item)
{
  switch (item)
  {
    case SPR_ARM1: // 136
