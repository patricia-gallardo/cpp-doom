...
void
  M_SizeDisplay(int choice); // villsa [STRIFE]
  ...
  ST_Responder(event_t *ev)
{
  ...
  if (ev->type == ev_keyup)
  {
    ...
    // villsa [STRIFE]
    if (ev->data1 != key_invpop && ev->data1 != key_mission && ev->data1 != key_invkey)
      return false;
  ...
  if (ev->data1 == key_invquery) // inventory query
  {
    ...
    if (inv->amount)
    {
      DEH_snprintf(st_msgbuf, sizeof(st_msgbuf), "%d %s", inv->amount, DEH_String(mobjinfo[inv->type].name));
...
#define FIRSTKEYSETAMOUNT 16
  ...
  if (cht_CheckCheat(&cheat_mypos, ev->data2))
  {
    ...
    M_snprintf(buf, sizeof(buf), "ang=0x%x;x,y=(0x%x,0x%x)", players[consoleplayer].mo->angle, players[consoleplayer].mo->x, players[consoleplayer].mo->y);
    plyr->message = buf;
  }
  ...
  if (cht_CheckCheat(&cheat_clev, ev->data2))
  {
    ...
    {
      if ((isdemoversion && (map < 32 || map > 34)) || (isregistered && (map <= 0 || map > 34)))
        return false;
    }
    else
    {
...
//
void
  ST_drawNumFontY(int x, int y, int num)
{
  if (!num)
...
//
void
  ST_drawNumFontY2(int x, int y, int num)
{
  if (!num)
    V_DrawPatch(x, y, invfonty[0]);
...
//
void
  ST_drawLine(int x, int y, int len, int color)
{
  ...
  ST_doRefresh()
{
  ...
  if (st_statusbaron)
  {
         ...
         num_x += 35, icon_x += 35, i++, numdrawn++)
    {
      ...
      DEH_snprintf(iconname, sizeof(iconname), "I_%s", DEH_String(sprnames[plyr->inventory[i].sprite]));
...
//
static void
  ST_drawTime(int x, int y, int time)
{
  int  hours;
  int  minutes;
  int  seconds;
  ...
  else
  {
    ...
    for (; keycount < ST_KEYSPERPAGE && key < NUMCARDS; ++key, ++keycount, ++info)
    {
      ...
      if (plyr->cards[key])
      {
        ...
        DEH_snprintf(sprname, sizeof(sprname), "I_%s", sprnames[states[info->spawnstate].sprite]);
  ...
  ST_DrawExternal()
{
  ...
  else
  {
    ...
    {
      V_DrawPatchDirect(38, 86, cache_lump_name<patch_t *>(DEH_String("CBOWA0"), PU_CACHE));
    }
    if (plyr->weaponowned[wp_rifle])
    {
      V_DrawPatchDirect(40, 107, cache_lump_name<patch_t *>(DEH_String("RIFLA0"), PU_CACHE));
    }
    if (plyr->weaponowned[wp_missile])
    {
      V_DrawPatchDirect(39, 131, cache_lump_name<patch_t *>(DEH_String("MMSLA0"), PU_CACHE));
    }
    if (plyr->weaponowned[wp_hegrenade])
    {
      V_DrawPatchDirect(78, 87, cache_lump_name<patch_t *>(DEH_String("GRNDA0"), PU_CACHE));
    }
    if (plyr->weaponowned[wp_flame])
    {
      V_DrawPatchDirect(80, 117, cache_lump_name<patch_t *>(DEH_String("FLAMA0"), PU_CACHE));
    }
    if (plyr->weaponowned[wp_mauler])
    {
      V_DrawPatchDirect(75, 142, cache_lump_name<patch_t *>(DEH_String("TRPDA0"), PU_CACHE));
    }
    ...
    {
      V_DrawPatchDirect(280, 130, cache_lump_name<patch_t *>(DEH_String("I_COMM"), PU_CACHE));
    }
  }
...
//
static void
  ST_loadUnloadGraphics(load_callback_t callback)
{
  int  i;
  char namebuf[9];
...
static void
  ST_loadCallback(const char *lumpname, patch_t **variable)
{
...
static void
  ST_unloadCallback(const char *lumpname, patch_t **variable)
{
  ...
  ST_createWidgets()
{
  ...
  for (i = 0; i < NUMAMMO; i++)
  {
    STlib_initNum(&w_ammo[i], ST_POPUPAMMOX, st_yforammo[i], invfonty, &plyr->ammo[i], st_wforammo[i]);

    STlib_initNum(&w_maxammo[i], ST_POPUPMAXAMMOX, st_yforammo[i], invfonty, &plyr->maxammo[i], st_wforammo[i]);
  }
}
