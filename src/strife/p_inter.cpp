  ...
  P_TouchSpecialThing(mobj_t *special, mobj_t *toucher)
{
  ...
  if (!pickupmsg[0])
  {
    ...
    {
      DEH_snprintf(pickupmsg, sizeof(pickupmsg), "You picked up the %s.", DEH_String(special->info->name));
    }
    else
  ...
  */
  ...
  if (source && source->player)
  {
    ...
    if (target->player)
    {
      ...
      DEH_snprintf(plrkilledmsg, sizeof(plrkilledmsg), "%s killed %s", player_names[source->player->allegiance], player_names[target->player->allegiance]);
  ...
  switch (item)
  {
    case MT_TOKEN_SHOPCLOSE:
      ...
      M_snprintf(plrkilledmsg, sizeof(plrkilledmsg), "%s", DEH_String("You're dead!  You set off the alarm."));
      if (!deathmatch)
        players[consoleplayer].message = plrkilledmsg;
    ...
    case MT_TOKEN_ALARM:
      ...
      M_snprintf(plrkilledmsg, sizeof(plrkilledmsg), "%s", DEH_String("You Fool!  You've set off the alarm"));
      if (!deathmatch)
        players[consoleplayer].message = plrkilledmsg;
      return;
  ...
  P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
  ...
  if (inflictor)
  {
    ...
    if (inflictor->type == MT_SFIREBALL || inflictor->type == MT_C_FLAME || inflictor->type == MT_PFLAME)
    {
      temp = damage / 2;
  ...
  if (target->health <= 0)
  {
    ...
    else if (!(target->flags & MF_NOBLOOD))
    {
      ...
      if (inflictor && (inflictor->type == MT_STRIFEPUFF3 || inflictor->type == MT_L_LASER || inflictor->type == MT_TORPEDO || inflictor->type == MT_TORPEDOSPREAD))
      {
    ...
    if (!(target->flags & MF_NOBLOOD)
        && inflictor
        && (inflictor->type == MT_SFIREBALL || inflictor->type == MT_C_FLAME || inflictor->type == MT_PFLAME))
    {
