...
//
static int
  S_GetChannel(mobj_t *origin, sfxinfo_t *sfxinfo, boolean isvoice)
{
  ...
  for (cnum = 0; cnum < snd_channels; cnum++)
  {
    ...
    else if (origin && channels[cnum].origin == origin && (isvoice || cnum != i_voicehandle)) // haleyjd
    {
...
void
  S_StartSound(void *origin_p, int sfx_id)
{
  sfxinfo_t *sfx;
  mobj_t    *origin;
  int        rc;
...
//
static voiceinfo_t *
  S_getVoice(const char *name, int lumpnum)
{
  voiceinfo_t *voice;
  ...
  I_StartVoice(const char *lumpname)
{
  ...
  if ((lumpnum = W_CheckNumForName(lumpnamedup)) != -1)
  {
    ...
    channels[i_voicehandle].handle = I_StartSound(&voice->sfx, i_voicehandle, snd_VoiceVolume, NORM_SEP, NORM_PITCH);
  }
}
