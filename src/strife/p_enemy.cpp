...
void
  P_ThrustMobj(mobj_t *actor, angle_t angle, fixed_t force);
...
//
static void
  P_WakeUpThing(mobj_t *puncher, mobj_t *bystander)
{
...
//
void
  P_DoPunchAlert(mobj_t *puncher, mobj_t *punchee)
{
  mobj_t *rover;
  ...
  for (rover = punchee->snext; rover; rover = rover->snext)
  {
    ...
    if (rover->health > 0 && rover->type >= MT_GUARD1 && rover->type <= MT_PGUARD && (P_CheckSight(rover, puncher) || P_CheckSight(rover, punchee)))
    {
...
//
boolean
  P_CheckRobotRange(mobj_t *actor)
{
  fixed_t dist;
                   ...
                   boolean allaround)
{
  ...
  if (actor->flags & MF_ALLY)
  {
    ...
    if (netgame)
    {
      ...
      if (master && master->target && (master->target->type != MT_REBEL1 || master->target->miscdata != actor->miscdata))
      {
  ...
  A_FriendLook(mobj_t *actor)
{
  ...
  if (soundtarget && soundtarget->flags & MF_SHOOTABLE)
  {
    ...
    if ((actor->flags & MF_ALLY) == (soundtarget->flags & MF_ALLY) && gamemap != 3 && gamemap != 34)
    {
      // STRIFE-TODO: Needs serious verification.
      if (P_LookForPlayers(actor, (actor->flags & MF_GIVEQUEST) != 0))
      {
...
nomissile:
  ...
  {
    if (actor->info->activesound >= sfx_agrac1 && actor->info->activesound <= sfx_agrac4)
    {
  ...
  A_CheckTargetVisible(mobj_t *actor)
{
  ...
  if (P_Random() >= 30)
  {
    ...
    if (!target || target->health <= 0 || !P_CheckSight(actor, target) || P_Random() < 40)
    {
      P_SetMobjState(actor, actor->info->seestate);
    }
  }
  ...
  A_InqFly(mobj_t *actor)
{
  ...
  if (--actor->reactiontime < 0 || !actor->momx || !actor->momy || actor->z <= actor->floorz)
  {
  ...
  A_ProgrammerAttack(mobj_t *actor)
{
  ...
  mo            = P_SpawnMobj(actor->target->x, actor->target->y, ONFLOORZ, MT_SIGIL_A_GROUND);
  mo->threshold = 25;
  mo->target    = actor;
  mo->health    = -2;
