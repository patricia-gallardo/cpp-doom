...
//
static void
  P_ParseDialogLump(byte *lump, mapdialog_t **dialogs, int numdialogs, int tag)
{
  int   i;
  byte *rover = lump;
...
//
int
  P_PlayerHasItem(player_t *player, mobjtype_t type)
{
  int i;
...
//
mapdialog_t *
  P_DialogFind(mobjtype_t type, int jumptoconv)
{
  int i;
...
//
static dialogstateset_t *
  P_DialogGetStates(mobjtype_t type)
{
  int i;
...
//
static const char *
  P_DialogGetMsg(const char *message)
{
...
//
boolean
  P_GiveInventoryItem(player_t *player, int sprnum, mobjtype_t type)
{
  int          curinv = 0;
  int          i;
  boolean      ok   = false;
...
//
boolean
  P_GiveItemToPlayer(player_t *player, int sprnum, mobjtype_t type)
{
  int    i = 0;
  line_t junk;
...
//
static void
  P_TakeDialogItem(player_t *player, int type, int amount)
{
  int i;
  ...
  P_DialogDrawer()
{
  ...
  if (dialogbgpiclumpnum == -1 || menupausetime <= gametic)
  {
    ...
    {
      DEH_snprintf(choicetext, sizeof(choicetext), "%d) %s", i + 1, currentdialog->choices[i].text);
      ...
      if (currentdialog->choices[i].needamounts[0] > 0)
      {
        ...
        DEH_snprintf(choicetext, sizeof(choicetext), "%s for %d", choicetext2, currentdialog->choices[i].needamounts[0]);
      }
  ...
  P_DialogStart(player_t *player)
{
  ...
  DEH_snprintf(dialoglastmsgbuffer, sizeof(dialoglastmsgbuffer), "%d) %s", i + 1, byetext);
}
