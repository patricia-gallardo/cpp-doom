...
//
void
  G_RiftExitLevel(int map, int spot, angle_t angle)
{
...
//
void
  G_Exit2(int dest, angle_t angle)
{
  riftdest   = dest;
  gameaction = ga_completed;
  riftangle  = angle;
  ...
  G_ReadCurrent(const char *path)
{
  ...
  else
  {
    ...
    gamemap    = (((int)buffer[0]) | ((int)buffer[1] << 8) | ((int)buffer[2] << 16) | ((int)buffer[3] << 24));
    gameaction = ga_loadgame;
    Z_Free(buffer);
  }
