...
const char    *cursorName[8] = { "M_CURS1", "M_CURS2", "M_CURS3", "M_CURS4", "M_CURS5", "M_CURS6", "M_CURS7", "M_CURS8" };
...
void
  M_StartMessage(const char *string, void *routine, boolean input);
void
  M_StopMessage();
...
menu_t MainDef = {
  ...
  M_DrawMainMenu,
  97,
  45, // haleyjd 08/28/10: [STRIFE] changed y coord
  0
};
...
menu_t ReadDef2 = {
  ...
  M_DrawReadThis2,
  250,
  185, // haleyjd 08/28/10: [STRIFE] changed coords
  0
};
...
menu_t SoundDef = {
  ...
  M_DrawSound,
  80,
  35, // [STRIFE] changed y coord 64 -> 35
  0
};
  ...
  M_ReadSaveStrings()
{
  ...
  for (i = 0; i < load_end; i++)
  {
    ...
    if (handle == NULL)
    {
      M_StringCopy(savegamestrings[i], DEH_String(EMPTYSTRING), sizeof(savegamestrings[i]));
      LoadMenu[i].status = 0;
      continue;
    }
  ...
  M_DrawSound()
{
  ...
  M_DrawThermo(SoundDef.x, SoundDef.y + LINEHEIGHT * (voice_vol + 1), 16, voiceVolume);

  M_DrawThermo(SoundDef.x, SoundDef.y + LINEHEIGHT * (sfx_mouse + 1), 16, mouseSensitivity);
}
...
void
  M_DrawMainMenu()
{
  V_DrawPatchDirect(84, 2, cache_lump_name<patch_t *>(DEH_String("M_STRIFE"), PU_CACHE));
}
  ...
  M_DrawOptions()
{
  ...
  V_DrawPatchDirect(108, 15, cache_lump_name<patch_t *>(DEH_String("M_OPTION"), PU_CACHE));
...
//
void
  M_QuitStrife(int choice)
{
  DEH_snprintf(endstring, sizeof(endstring), "Do you really want to leave?\n\n" DOSY);
               ...
               int thermDot)
{
  ...
  V_DrawPatchDirect((x + 8) + thermDot * 8, y + 2, cache_lump_name<patch_t *>(DEH_String("M_THERMO"), PU_CACHE));
}
...
void
  M_DrawEmptyCell(menu_t *menu,
                  int     item)
{
  V_DrawPatchDirect(menu->x - 10, menu->y + item * LINEHEIGHT - 1, cache_lump_name<patch_t *>(DEH_String("M_CELL1"), PU_CACHE));
}
...
void
  M_DrawSelCell(menu_t *menu,
                int     item)
{
  V_DrawPatchDirect(menu->x - 10, menu->y + item * LINEHEIGHT - 1, cache_lump_name<patch_t *>(DEH_String("M_CELL2"), PU_CACHE));
}
  ...
  if (saveStringEnter)
  {
    switch (key)
    {
      ...
      case KEY_ESCAPE:
        saveStringEnter = 0;
        I_StopTextInput();
        M_StringCopy(savegamestrings[quickSaveSlot], saveOldString, sizeof(savegamestrings[quickSaveSlot]));
        break;
      ...
      default:
        ...
        if (ch >= 32 && ch <= 127 && saveCharIndex < SAVESTRINGSIZE - 1 && M_StringWidth(savegamestrings[quickSaveSlot]) < (SAVESTRINGSIZE - 2) * 8)
        {
  ...
  */
  ...
  if (!menuactive)
  {
    ...
    else if (key == key_menu_save) // Save
    {
      ...
      if (netgame || players[consoleplayer].health <= 0 || players[consoleplayer].cheats & CF_ONFIRE)
      {
        S_StartSound(NULL, sfx_oof);
      }
      else
  ...
  M_Drawer()
{
  ...
  // DRAW SIGIL
  V_DrawPatchDirect(x + CURSORXOFF, currentMenu->y - 5 + itemOn * LINEHEIGHT, cache_lump_name<patch_t *>(DEH_String(cursorName[whichCursor]), PU_CACHE));
}
