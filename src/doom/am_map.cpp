...
void
  AM_rotate(int64_t *x, int64_t *y, angle_t a);
static void
  ...
  for (i = 0; i < numsectors; i++)
  {
    ...
    while (t)
    {
      ...
      if (crispy->extautomap)
      {
        ...
        if (key > no_key)
        {
          AM_drawLineCharacter(cross_mark, arrlen(cross_mark), 16 << FRACBITS, t->angle, (key == red_key) ? REDS : (key == yellow_key) ? YELLOWS :
                                                                                                                 (key == blue_key)     ? BLUES :
                                                                                                                                         colors + lightlev,
                               pt.x,
                               pt.y);
          ...
          {
            AM_drawLineCharacter(square_mark, arrlen(square_mark), t->radius >> 2, t->angle, (t->type == MT_BLOOD) ? REDS : GRAYS, pt.x, pt.y);
          }
          else
          {
