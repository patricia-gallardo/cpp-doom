...
static void
  CrispyReplaceColor(char *str, const int cr, const char *col)
{
...
static void
  HU_SetSpecialLevelName(const char *wad, const char **name)
{
  int i;
  ...
  HU_Ticker()
{
  ...
  if (crispy->automapstats == WIDGETS_ALWAYS || (automapactive && crispy->automapstats == WIDGETS_AUTOMAP))
  {
    ...
    if (extrakills)
      M_snprintf(str, sizeof(str), "%s%s%s%d/%d+%d", cr_stat, kills, crstr[static_cast<int>(cr_t::CR_GRAY)], plr->killcount, totalkills, extrakills);
    else
      M_snprintf(str, sizeof(str), "%s%s%s%d/%d", cr_stat, kills, crstr[static_cast<int>(cr_t::CR_GRAY)], plr->killcount, totalkills);
    HUlib_clearTextLine(&w_kills);
    s = str;
    while (*s)
      HUlib_addCharToTextLine(&w_kills, *(s++));

    M_snprintf(str, sizeof(str), "%sI %s%d/%d", cr_stat, crstr[static_cast<int>(cr_t::CR_GRAY)], plr->itemcount, totalitems);
    HUlib_clearTextLine(&w_items);
    s = str;
    while (*s)
      HUlib_addCharToTextLine(&w_items, *(s++));

    M_snprintf(str, sizeof(str), "%sS %s%d/%d", cr_stat, crstr[static_cast<int>(cr_t::CR_GRAY)], plr->secretcount, totalsecret);
