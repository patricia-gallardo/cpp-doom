//
// Copyright(C) 1993-1996 Id Software, Inc.
// Copyright(C) 2005-2014 Simon Howard
//
// This program is free software; you can redistribute it and/or
// modify it under the terms of the GNU General Public License
// as published by the Free Software Foundation; either version 2
// of the License, or (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU General Public License for more details.
//
// DESCRIPTION:
//	Created by the sound utility written by Dave Taylor.
//	Kept as a sample, DOOM2  sounds. Frozen.
//

#ifndef __SOUNDS__
#define __SOUNDS__

#include "i_sound.hpp"

// the complete set of sound effects
extern sfxinfo_t S_sfx[];

// the complete set of music
extern musicinfo_t S_music[];

//
// Identifiers for all music in game.
//

enum musicenum_t
{
    mus_None,
    mus_e1m1,
    mus_e1m2 [[maybe_unused]],
    mus_e1m3 [[maybe_unused]],
    mus_e1m4 [[maybe_unused]],
    mus_e1m5,
    mus_e1m6 [[maybe_unused]],
    mus_e1m7 [[maybe_unused]],
    mus_e1m8 [[maybe_unused]],
    mus_e1m9,
    mus_e2m1 [[maybe_unused]],
    mus_e2m2 [[maybe_unused]],
    mus_e2m3 [[maybe_unused]],
    mus_e2m4,
    mus_e2m5,
    mus_e2m6,
    mus_e2m7,
    mus_e2m8 [[maybe_unused]],
    mus_e2m9 [[maybe_unused]],
    mus_e3m1 [[maybe_unused]],
    mus_e3m2,
    mus_e3m3,
    mus_e3m4,
    mus_e3m5 [[maybe_unused]],
    mus_e3m6 [[maybe_unused]],
    mus_e3m7 [[maybe_unused]],
    mus_e3m8 [[maybe_unused]],
    mus_e3m9 [[maybe_unused]],
    // [crispy] support dedicated music tracks for the 4th episode
    mus_e4m1,
    mus_e4m2 [[maybe_unused]],
    mus_e4m3 [[maybe_unused]],
    mus_e4m4 [[maybe_unused]],
    mus_e4m5 [[maybe_unused]],
    mus_e4m6 [[maybe_unused]],
    mus_e4m7 [[maybe_unused]],
    mus_e4m8 [[maybe_unused]],
    mus_e4m9 [[maybe_unused]],
    // [crispy] Sigil
    mus_e5m1 [[maybe_unused]],
    mus_e5m2 [[maybe_unused]],
    mus_e5m3 [[maybe_unused]],
    mus_e5m4 [[maybe_unused]],
    mus_e5m5 [[maybe_unused]],
    mus_e5m6 [[maybe_unused]],
    mus_e5m7 [[maybe_unused]],
    mus_e5m8 [[maybe_unused]],
    mus_e5m9,
    mus_sigint,
    mus_inter,
    mus_intro,
    mus_bunny,
    mus_victor,
    mus_introa,
    mus_runnin,
    mus_stalks [[maybe_unused]],
    mus_countd [[maybe_unused]],
    mus_betwee [[maybe_unused]],
    mus_doom,
    mus_the_da,
    mus_shawn,
    mus_ddtblu,
    mus_in_cit,
    mus_dead [[maybe_unused]],
    mus_stlks2 [[maybe_unused]],
    mus_theda2 [[maybe_unused]],
    mus_doom2 [[maybe_unused]],
    mus_ddtbl2,
    mus_runni2 [[maybe_unused]],
    mus_dead2 [[maybe_unused]],
    mus_stlks3 [[maybe_unused]],
    mus_romero [[maybe_unused]],
    mus_shawn2,
    mus_messag,
    mus_count2 [[maybe_unused]],
    mus_ddtbl3 [[maybe_unused]],
    mus_ampie [[maybe_unused]],
    mus_theda3 [[maybe_unused]],
    mus_adrian [[maybe_unused]],
    mus_messg2 [[maybe_unused]],
    mus_romer2 [[maybe_unused]],
    mus_tense [[maybe_unused]],
    mus_shawn3 [[maybe_unused]],
    mus_openin [[maybe_unused]],
    mus_evil,
    mus_ultima [[maybe_unused]],
    mus_read_m,
    mus_dm2ttl,
    mus_dm2int,
    NUMMUSIC,
    mus_musinfo
};


//
// Identifiers for all sfx in game.
//

enum sfxenum_t
{
    sfx_None,
    sfx_pistol,
    sfx_shotgn,
    sfx_sgcock,
    sfx_dshtgn,
    sfx_dbopn,
    sfx_dbcls,
    sfx_dbload,
    sfx_plasma,
    sfx_bfg,
    sfx_sawup,
    sfx_sawidl,
    sfx_sawful,
    sfx_sawhit,
    sfx_rlaunc,
    sfx_rxplod,
    sfx_firsht,
    sfx_firxpl,
    sfx_pstart,
    sfx_pstop,
    sfx_doropn,
    sfx_dorcls,
    sfx_stnmov,
    sfx_swtchn,
    sfx_swtchx,
    sfx_plpain,
    sfx_dmpain,
    sfx_popain,
    sfx_vipain,
    sfx_mnpain,
    sfx_pepain,
    sfx_slop,
    sfx_itemup,
    sfx_wpnup,
    sfx_oof,
    sfx_telept,
    sfx_posit1,
    sfx_posit2,
    sfx_posit3,
    sfx_bgsit1,
    sfx_bgsit2,
    sfx_sgtsit,
    sfx_cacsit,
    sfx_brssit,
    sfx_cybsit,
    sfx_spisit,
    sfx_bspsit,
    sfx_kntsit,
    sfx_vilsit,
    sfx_mansit,
    sfx_pesit,
    sfx_sklatk,
    sfx_sgtatk,
    sfx_skepch,
    sfx_vilatk,
    sfx_claw,
    sfx_skeswg,
    sfx_pldeth,
    sfx_pdiehi,
    sfx_podth1,
    sfx_podth2,
    sfx_podth3,
    sfx_bgdth1,
    sfx_bgdth2,
    sfx_sgtdth,
    sfx_cacdth,
    sfx_skldth [[maybe_unused]],
    sfx_brsdth,
    sfx_cybdth,
    sfx_spidth,
    sfx_bspdth,
    sfx_vildth,
    sfx_kntdth,
    sfx_pedth,
    sfx_skedth,
    sfx_posact,
    sfx_bgact,
    sfx_dmact,
    sfx_bspact,
    sfx_bspwlk,
    sfx_vilact,
    sfx_noway,
    sfx_barexp,
    sfx_punch,
    sfx_hoof,
    sfx_metal,
    sfx_chgun [[maybe_unused]],
    sfx_tink,
    sfx_bdopn,
    sfx_bdcls,
    sfx_itmbk,
    sfx_flame,
    sfx_flamst,
    sfx_getpow,
    sfx_bospit,
    sfx_boscub,
    sfx_bossit,
    sfx_bospn,
    sfx_bosdth,
    sfx_manatk,
    sfx_mandth,
    sfx_sssit,
    sfx_ssdth,
    sfx_keenpn,
    sfx_keendt,
    sfx_skeact,
    sfx_skesit,
    sfx_skeatk,
    sfx_radio,
    // [crispy] additional BOOM and MBF states, sprites and code pointers
    sfx_dgsit,
    sfx_dgatk,
    sfx_dgact,
    sfx_dgdth,
    sfx_dgpain,
    // [crispy] play DSSECRET if available
    sfx_secret,
    NUMSFX
};

#endif
