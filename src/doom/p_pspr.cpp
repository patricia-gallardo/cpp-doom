...
static void
  DecreaseAmmo(player_t *player, int ammonum, int amount)
{
  if (ammonum < NUMAMMO)
  {
...
//
void
  A_Light0(mobj_t *mobj, player_t *player, pspdef_t *psp)
{
  if (!player) return; // [crispy] let pspr action pointers get called from mobj states
  player->extralight = 0;
}
...
void
  A_Light1(mobj_t *mobj, player_t *player, pspdef_t *psp)
{
  if (!player) return; // [crispy] let pspr action pointers get called from mobj states
  player->extralight = 1;
}
...
void
  A_Light2(mobj_t *mobj, player_t *player, pspdef_t *psp)
{
  if (!player) return; // [crispy] let pspr action pointers get called from mobj states
  player->extralight = 2;
}
