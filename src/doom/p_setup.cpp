...
fixed_t
  GetOffset(vertex_t *v1, vertex_t *v2)
{
  fixed_t dx, dy;
  fixed_t r;
...
static void
  PadRejectArray(byte *array, unsigned int len)
{
  unsigned int i;
...
int
  P_GetNumForMap(int episode, int map, boolean critical)
{
  char lumpname[9];
  int  lumpnum;
...
*/
  ...
  // [crispy] better logging
  {
    ...
    fprintf(stderr, "P_SetupLevel: %s (%s) %s%s %d:%02d:%02d/%d:%02d:%02d ", maplumpinfo->name, W_WadNameForLump(maplumpinfo), skilltable[BETWEEN(0, 5, (int)skill + 1)], rfn_str, ltime / 3600, (ltime % 3600) / 60, ltime % 60, ttime / 3600, (ttime % 3600) / 60, ttime % 60);
