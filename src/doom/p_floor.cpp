...
void T_MoveGoobers(floormove_t * floor) {
  ...
  result_e res2 = T_MovePlane(floor->sector, 2 * FLOORSPEED, floor->floordestheight, true, 1, (floor->direction >> 1) * 2 - 1);
...
int EV_DoFloor(line_t * line,
  ...
  while ((secnum = P_FindSectorFromLineTag(line, secnum)) >= 0) {
    ...
    case turboLower:
      ...
      floor->floordestheight =
          P_FindHighestFloorSurrounding(sec);
      if (g_doomstat_globals->gameversion <= exe_doom_1_2 || floor->floordestheight != sec->floorheight)
