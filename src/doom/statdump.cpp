...
static void
  DiscoverGamemode(const wbstartstruct_t *stats, int num_stats)
{
  int partime;
  int level;
  int i;
...
static int
  GetNumPlayers(const wbstartstruct_t *stats)
{
  int i;
  int num_players = 0;
...
static void
  PrintBanner(FILE *stream)
{
  fprintf(stream, "===========================================\n");
}

static void
  PrintPercentage(FILE *stream, int amount, int total)
{
  if (total == 0)
  {
    fprintf(stream, "0");
...
static void
  PrintPlayerStats(FILE *stream, const wbstartstruct_t *stats, int player_num)
{
...
static void
  PrintFragsTable(FILE *stream, const wbstartstruct_t *stats)
{
  int x, y;
...
static void
  PrintLevelName(FILE *stream, int episode, int level)
{
  PrintBanner(stream);
...
static void
  PrintStats(FILE *stream, const wbstartstruct_t *stats)
{
...
void
  StatCopy(const wbstartstruct_t *stats)
{
