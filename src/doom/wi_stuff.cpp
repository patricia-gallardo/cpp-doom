...
void WI_drawLF() {
  ...
  // [crispy] prevent crashes with maps without map title graphics lump
  if (wbs->last >= static_cast<int>(num_lnames) || lnames[wbs->last] == nullptr) {
...
void WI_drawEL() {
  ...
  // [crispy] prevent crashes with maps without map title graphics lump
  if (wbs->next >= static_cast<int>(num_lnames) || lnames[wbs->next] == nullptr) {
    return;
  }
...
int WI_fragSum(int playernum) {
  ...
  frags_local -= plrs[playernum].frags[playernum];
  // UNUSED if (frags < 0)
  // 	frags = 0;
...
static void WI_loadUnloadData(load_callback_t callback) {
  ...
  if (g_doomstat_globals->nervewadfile && g_doomstat_globals->gamemission == pack_nerve) {
    ...
    for (index = 0; index < 9; index++) {
      DEH_snprintf(name, 9, "NWILV%2.2d", index);
      callback(name, &lnames[index]);
    }
    for (; index < NUMCMAPS; index++) {
      DEH_snprintf(name, 9, "CWILV%2.2d", index);
      callback(name, &lnames[index]);
    }
    ...
    for (index = 0; index < NUMMAPS; index++) {
      DEH_snprintf(name, 9, "WILV%d%d", wbs->epsd, index);
      callback(name, &lnames[index]);
    }
    ...
    if (wbs->epsd < 3) {
      for (int j = 0; j < NUMANIMS[wbs->epsd]; j++) {
        ...
        for (index = 0; index < a->nanims; index++) {
          ...
          if (wbs->epsd != 1 || j != 8) {
            // animations
            DEH_snprintf(name, 9, "WIA%d%.2d%.2d", wbs->epsd, j, index);
            callback(name, &a->p[index]);
          } else {
            // HACK ALERT!
            a->p[index] = anims[1][4].p[index];
          }
        }
      }
    }
...
void WI_loadData() {
  if (g_doomstat_globals->gamemode == commercial) {
    ...
    lnames      = zmalloc<patch_t **>(sizeof(patch_t *) * static_cast<unsigned long>(nummaps),
                                 PU_STATIC,
                                 nullptr);
