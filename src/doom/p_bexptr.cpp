...
void
  A_BetaSkullAttack(mobj_t *actor)
{
  int damage;
  ...
  A_Spawn(mobj_t *mo)
{
  if (mo->state->misc1)
  {
    /*    mobj_t *newmobj = */ P_SpawnMobj(mo->x, mo->y, (mo->state->misc2 << FRACBITS) + mo->z, static_cast<mobjtype_t>(mo->state->misc1 - 1));
...
void
  A_FireOldBFG(mobj_t *mobj, player_t *player, pspdef_t *psp)
{
