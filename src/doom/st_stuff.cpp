...
static inline int
  cht_CheckCheatSP(cheatseq_t *cht, char key)
{
...
#endif
      M_snprintf(msg, sizeof(msg), "%s (%s) x%ld SDL%s", PACKAGE_STRING, BUILD_DATE, (long)sizeof(void *) * CHAR_BIT, crispy->sdlversion);
#undef BUILD_DATE
      plyr->message = msg;
      fprintf(stderr, "%s\n", msg);
    }
