  ...
  D_Display()
{
  ...
  if (paused)
  {
    ...
    V_DrawPatchDirect((viewwindowx >> crispy->hires) + ((scaledviewwidth >> crispy->hires) - 68) / 2 - DELTAWIDTH, y, cache_lump_name<patch_t *>(DEH_String("M_PAUSE"), PU_CACHE));
  }
...
void
  EnableLoadingDisk() // [crispy] un-static
{
...
static const char *
  GetGameName(const char *gamename)
{
  size_t      i;
  const char *deh_sub;
...
static void
  SetMissionForPackName(const char *pack_name)
{
