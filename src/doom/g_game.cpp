  ...
  G_BuildTiccmd(ticcmd_t *cmd, int maketic)
{
  ...
  if (gamekeydown[key_toggleautorun])
  {
    ...
    M_snprintf(playermessage, sizeof(playermessage), "ALWAYS RUN %s%s", crstr[static_cast<int>(cr_t::CR_GREEN)], (joybspeed >= MAX_JOY_BUTTONS) ? "ON" : "OFF");
  ...
  {
    M_snprintf(playermessage, sizeof(playermessage), "X=%.10f Y=%.10f A=%d", (double)player->mo->x / FRACUNIT, (double)player->mo->y / FRACUNIT, player->mo->angle >> 24);
    player->message = playermessage;
...
static void
  SetJoyButtons(unsigned int buttons_mask)
{
  int i;
...
static void
  SetMouseButtons(unsigned int buttons_mask)
{
  int i;
  ...
  G_DoSaveGame()
{
  ...
  // [crispy] some logging when saving
  {
    ...
    fprintf(stderr, "G_DoSaveGame: Episode %d, Map %d, %s, Time %d:%02d:%02d, Total %d:%02d:%02d.\n", gameepisode, gamemap, skilltable[BETWEEN(0, 5, (int)gameskill + 1)], ltime / 3600, (ltime % 3600) / 60, ltime % 60, ttime / 3600, (ttime % 3600) / 60, ttime % 60);
  }
...
static const char *
  DemoVersionDescription(int version)
{
