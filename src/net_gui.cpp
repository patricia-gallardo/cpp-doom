...
static void
  EscapePressed(TXT_UNCAST_ARG(widget), void *unused)
{
  TXT_Shutdown();
  I_Quit();
}
...
static void
  StartGame(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
  NET_CL_LaunchGame();
}
...
static void
  PrintSHA1Digest(const char *s, const byte *digest)
{
  unsigned int i;
...
static void
  CloseWindow(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(window))
{
