...
static void UpdateGUI() {
  ...
  if (g_net_client_globals->net_client_wait_data.num_drones > 0) {
    M_snprintf(buf, sizeof(buf), " (+%i observer clients)", g_net_client_globals->net_client_wait_data.num_drones);
...
static void BuildMasterStatusWindow() {
  ...
  TXT_SetWindowPosition(master_window, TXT_HORIZ_CENTER, TXT_VERT_CENTER, TXT_SCREEN_W / 2, TXT_SCREEN_H - 4);
...
static void CheckSHA1Sums() {
  ...
  bool correct_wad = memcmp(g_net_client_globals->net_local_wad_sha1sum, g_net_client_globals->net_client_wait_data.wad_sha1sum, sizeof(sha1_digest_t))
                     == 0;
  bool correct_deh = memcmp(g_net_client_globals->net_local_deh_sha1sum, g_net_client_globals->net_client_wait_data.deh_sha1sum, sizeof(sha1_digest_t))
                     == 0;
  bool same_freedoom =
