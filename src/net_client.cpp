...
static void NET_CL_AdvanceWindow() {
  ...
  while (recvwindow[0].active) {
    ...
    NET_CL_ExpandFullTiccmd(&recvwindow[0].cmd, static_cast<unsigned int>(recvwindow_start), ticcmds);
    D_ReceiveTic(ticcmds, recvwindow[0].cmd.playeringame);

    // Advance the window

    std::memmove(recvwindow, recvwindow + 1, sizeof(net_server_recv_t) * (BACKUPTICS - 1));
...
void NET_CL_StartGame(net_gamesettings_t * settings_param) {
  ...
  NET_WriteSettings(packet, settings_param);
}
