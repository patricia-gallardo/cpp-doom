...
static void
  NET_SV_DisconnectClient(net_client_t *client)
{
  if (client->active)
  {
...
static boolean
  ClientConnected(net_client_t *client)
{
...
static void
  NET_SV_SendConsoleMessage(net_client_t *client, const char *s, ...) PRINTF_ATTR(2, 3);
static void
  NET_SV_SendConsoleMessage(net_client_t *client, const char *s, ...)
{
  char          buf[1024];
  va_list       args;
  net_packet_t *packet;
...
static void
  NET_SV_BroadcastMessage(const char *s, ...) PRINTF_ATTR(1, 2);
static void
  NET_SV_BroadcastMessage(const char *s, ...)
{
  char    buf[1024];
  va_list args;
  int     i;
...
static void
  NET_SV_SendWaitingData(net_client_t *client)
{
...
static net_client_t *
  NET_SV_FindClient(net_addr_t *addr)
{
  int i;
...
static void
  NET_SV_SendReject(net_addr_t *addr, const char *msg)
{
  net_packet_t *packet;
...
static void
  NET_SV_InitNewClient(net_client_t *client, net_addr_t *addr, net_protocol_t protocol)
{
  client->active       = true;
...
static void
  NET_SV_ParseSYN(net_packet_t *packet, net_client_t *client, net_addr_t *addr)
{
  unsigned int       magic;
...
static void
  NET_SV_ParseLaunch(net_packet_t *packet, net_client_t *client)
{
...
static void
  NET_SV_ParseGameStart(net_packet_t *packet, net_client_t *client)
{
...
static void
  NET_SV_SendResendRequest(net_client_t *client, int start, int end)
{
  net_packet_t      *packet;
...
static void
  NET_SV_CheckResends(net_client_t *client)
{
  int          i;
  int          player;
...
static void
  NET_SV_ParseGameData(net_packet_t *packet, net_client_t *client)
{
  net_client_recv_t *recvobj;
  unsigned int       seq;
  unsigned int       ackseq;
...
static void
  NET_SV_ParseGameDataACK(net_packet_t *packet, net_client_t *client)
{
  unsigned int ackseq;
...
static void
  NET_SV_SendTics(net_client_t *client,
                  unsigned int  start,
                  unsigned int  end)
{
  net_packet_t *packet;
...
static void
  NET_SV_ParseResendRequest(net_packet_t *packet, net_client_t *client)
{
...
void
  NET_SV_SendQueryResponse(net_addr_t *addr)
{
  net_packet_t   *reply;
...
static void
  NET_SV_ParseHolePunch(net_packet_t *packet)
{
...
static void
  NET_SV_MasterPacket(net_packet_t *packet)
{
...
static void
  NET_SV_Packet(net_packet_t *packet, net_addr_t *addr)
{
  net_client_t *client;
  unsigned int  packet_type;
...
static void
  NET_SV_PumpSendQueue(net_client_t *client)
{
  net_full_ticcmd_t cmd;
  int               recv_index;
  int               num_players;
...
void
  NET_SV_CheckDeadlock(net_client_t *client)
{
  int nowtime;
  int i;
...
static void
  NET_SV_RunClient(net_client_t *client)
{
  // Run common code
