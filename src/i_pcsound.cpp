...
static void
  PCSCallbackFunc(int *duration, int *freq)
{
  unsigned int tone;
...
static boolean
  CachePCSLump(sfxinfo_t *sfxinfo)
{
  int lumplen;
  int headerlen;
...
static boolean
  IsDisabledSound(sfxinfo_t *sfxinfo)
{
  int         i;
...
static int
  I_PCS_StartSound(sfxinfo_t *sfxinfo,
                   int        channel,
                   int        vol,
                   int        sep,
                   int        pitch)
...
static int
  I_PCS_GetSfxLumpNum(sfxinfo_t *sfx)
{
  char namebuf[9];
...
static boolean
  I_PCS_SoundIsPlaying(int handle)
{
  if (!pcs_initialized)
  {
    return false;
...
static boolean
  I_PCS_InitSound(boolean _use_sfx_prefix)
{
...
void
  I_PCS_UpdateSoundParams(int channel, int vol, int sep)
{
  // no-op.
}
