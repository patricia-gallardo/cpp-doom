...
[[maybe_unused]] static bool opl_stereo_correct = false;
...
static void SetVoiceInstrument(opl_voice_t *     voice,
  ...
  unsigned int modulating = (data->feedback & 0x01) == 0;
...
static void SetChannelVolume(opl_channel_data_t * channel, unsigned int volume, bool clip_start);
...
static opl_channel_data_t * TrackChannelForEvent(opl_track_data_t *,
                                                 midi_event_t * event) {
...
static unsigned int FrequencyForVoice(opl_voice_t * voice) {
  auto note = static_cast<int>(voice->note);
  ...
  auto sub_index = static_cast<unsigned int>((freq_index - 284) % (12 * 32));
  auto octave    = static_cast<unsigned int>((freq_index - 284) / (12 * 32));
...
static void AllNotesOff(opl_channel_data_t * channel, unsigned int) {
  for (int i = 0; i < voice_alloced_num; i++) {
    if (voice_alloced_list[i]->channel == channel) {
      // Finished with this voice now.
...
static void PitchBendEvent(opl_track_data_t * track, midi_event_t * event) {
  ...
  for (unsigned int i = 0; i < voice_not_updated_num; i++) {
    voice_alloced_list[i] = voice_not_updated_list[i];
  }

  for (unsigned int i = 0; i < voice_updated_num; i++) {
...
[[maybe_unused]] void I_OPL_DevMessages(char * result, size_t result_len) {
  if (num_tracks == 0) {
