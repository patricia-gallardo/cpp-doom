...
static void
  VoiceKeyOff(opl_voice_t *voice);

static void
  ReleaseVoice(int index)
{
  opl_voice_t *voice;
...
static void
  LoadOperatorData(int op, genmidi_op_t *data, boolean max_level, unsigned int *volume)
{
  int level;
...
static void
  SetVoiceInstrument(opl_voice_t     *voice,
...
static void
  SetVoiceVolume(opl_voice_t *voice, unsigned int volume)
{
...
static void
  SetVoicePan(opl_voice_t *voice, unsigned int pan)
{
  genmidi_voice_t *opl_voice;
...
static void
  SetChannelVolume(opl_channel_data_t *channel, unsigned int volume, boolean clip_start);

// Set music volume (0 - 127)

static void
  I_OPL_SetMusicVolume(int volume)
{
  unsigned int i;
...
static opl_channel_data_t *
  TrackChannelForEvent(opl_track_data_t *track,
                       midi_event_t     *event)
{
...
static void
  KeyOffEvent(opl_track_data_t *track, midi_event_t *event)
{
  opl_channel_data_t *channel;
  int                 i;
  unsigned int        key;
...
static void
  ReplaceExistingVoiceDoom2(opl_channel_data_t *channel)
{
  int i;
  int result;
  int priority;
...
static unsigned int
  FrequencyForVoice(opl_voice_t *voice)
{
...
static void
  UpdateVoiceFrequency(opl_voice_t *voice)
{
  unsigned int freq;
...
static void
  VoiceKeyOn(opl_channel_data_t *channel,
             genmidi_instr_t    *instrument,
...
static void
  KeyOnEvent(opl_track_data_t *track, midi_event_t *event)
{
  genmidi_instr_t    *instrument;
  opl_channel_data_t *channel;
...
static void
  ProgramChangeEvent(opl_track_data_t *track, midi_event_t *event)
{
  opl_channel_data_t *channel;
  int                 instrument;
...
static void
  SetChannelPan(opl_channel_data_t *channel, unsigned int pan)
{
  unsigned int reg_pan;
  unsigned int i;
...
static void
  AllNotesOff(opl_channel_data_t *channel, unsigned int param)
{
  int i;
...
static void
  ControllerEvent(opl_track_data_t *track, midi_event_t *event)
{
  opl_channel_data_t *channel;
...
static void
  PitchBendEvent(opl_track_data_t *track, midi_event_t *event)
{
  opl_channel_data_t *channel;
  int                 i;
...
static void
  MetaSetTempo(unsigned int tempo)
{
...
static void
  MetaEvent(opl_track_data_t *track, midi_event_t *event)
{
...
static void
  ProcessEvent(opl_track_data_t *track, midi_event_t *event)
{
...
static void
  ScheduleTrack(opl_track_data_t *track);
static void
  InitChannel(opl_channel_data_t *channel);

// Restart a song from the beginning.

static void
  RestartSong(void *unused)
{
  unsigned int i;
...
static void
  StartTrack(midi_file_t *file, unsigned int track_num)
{
...
static void
  I_OPL_PlaySong(void *handle, boolean looping)
{
  unsigned int i;
...
static void
  I_OPL_UnRegisterSong(void *handle)
{
  if (!music_initialized)
  {
    return;
...
static void *
  I_OPL_RegisterSong(void *data, int len)
{
  midi_file_t *result;
  char        *filename;
...
static int
  ChannelInUse(opl_channel_data_t *channel)
{
  int i;
...
void
  I_OPL_DevMessages(char *result, size_t result_len)
{
  char tmp[80];
  int  instr_num;
  int  lines;
