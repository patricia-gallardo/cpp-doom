 ...
 */
static void
  Transform(sha1_context_t *hd, byte *data)
{
 ...
 */
void
  SHA1_Update(sha1_context_t *hd, byte *inbuf, size_t inlen)
{
  if (hd->count == 64)
  {
    /* flush the buffer */
...
void
  SHA1_Final(sha1_digest_t digest, sha1_context_t *hd)
{
  uint32_t t, msb, lsb;
  byte    *p;
