...
static boolean
  CheckChunkHeader(chunk_header_t *chunk,
...
static boolean
  ReadByte(byte *result, FILE *stream)
{
  int c;
...
static boolean
  ReadVariableLength(unsigned int *result, FILE *stream)
{
  int  i;
  byte b  = 0;
...
static byte *
  ReadByteSequence(unsigned int num_bytes, FILE *stream)
{
  unsigned int i;
  byte        *result;
...
static boolean
  ReadChannelEvent(midi_event_t *event,
                   byte          event_type,
                   boolean       two_param,
                   FILE         *stream)
{
...
static boolean
  ReadSysExEvent(midi_event_t *event, int event_type, FILE *stream)
{
...
static boolean
  ReadMetaEvent(midi_event_t *event, FILE *stream)
{
  byte b            = 0;
...
static boolean
  ReadEvent(midi_event_t *event, unsigned int *last_event_type, FILE *stream)
{
  byte event_type = 0;
...
static boolean
  ReadTrackHeader(midi_track_t *track, FILE *stream)
{
  size_t         records_read;
  chunk_header_t chunk_header;
...
static boolean
  ReadTrack(midi_track_t *track, FILE *stream)
{
...
static boolean
  ReadAllTracks(midi_file_t *file, FILE *stream)
{
  unsigned int i;
...
static boolean
  ReadFileHeader(midi_file_t *file, FILE *stream)
{
  size_t       records_read;
...
midi_track_iter_t *
  MIDI_IterateTrack(midi_file_t *file, unsigned int track)
{
...
static char *
  MIDI_EventTypeToString(midi_event_type_t event_type)
{
  switch (event_type)
  {
    case MIDI_EVENT_NOTE_OFF:
