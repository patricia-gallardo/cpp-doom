...
constexpr auto MIDIPIPE_MAX_WAIT = 1000; // Max amount of ms to wait for expected data.
...
static bool ExpectPipe(net_packet_t * packet) {
  ...
  do {
    ...
    ok = PeekNamedPipe(midi_process_out_reader, nullptr, 0, nullptr, &pipe_buffer_read, nullptr);
    if (!ok) {
      break;
    ...
    ok = ReadFile(midi_process_out_reader, pipe_buffer, static_cast<DWORD>(packet->len), &pipe_buffer_read, nullptr);
...
bool I_MidiPipe_InitServer() {
  ...
  ok = CreateProcess(TEXT(module), TEXT(cmdline), nullptr, nullptr, TRUE, 0, nullptr, dirname, &startup_info, &proc_info);
