...
static void
  QueueInit(packet_queue_t *queue)
{
  queue->head = queue->tail = 0;
}

static void
  QueuePush(packet_queue_t *queue, net_packet_t *packet)
{
  int new_tail;
...
static void
  NET_CL_SendPacket(net_addr_t *addr, net_packet_t *packet)
{
  QueuePush(&server_queue, NET_PacketDup(packet));
}

static boolean
  NET_CL_RecvPacket(net_addr_t **addr, net_packet_t **packet)
{
  net_packet_t *popped;
...
static void
  NET_CL_AddrToString(net_addr_t *addr, char *buffer, int buffer_len)
{
  M_snprintf(buffer, buffer_len, "local server");
}

static void
  NET_CL_FreeAddress(net_addr_t *addr)
{
}

static net_addr_t *
  NET_CL_ResolveAddress(const char *address)
{
  if (address == NULL)
  {
    client_addr.module = &net_loop_client_module;
...
static void
  NET_SV_SendPacket(net_addr_t *addr, net_packet_t *packet)
{
  QueuePush(&client_queue, NET_PacketDup(packet));
}

static boolean
  NET_SV_RecvPacket(net_addr_t **addr, net_packet_t **packet)
{
  net_packet_t *popped;
...
static void
  NET_SV_AddrToString(net_addr_t *addr, char *buffer, int buffer_len)
{
  M_snprintf(buffer, buffer_len, "local client");
}

static void
  NET_SV_FreeAddress(net_addr_t *addr)
{
}

static net_addr_t *
  NET_SV_ResolveAddress(const char *address)
{
  if (address == NULL)
  {
    server_addr.module = &net_loop_server_module;
