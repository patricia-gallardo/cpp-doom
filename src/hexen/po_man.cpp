...
static polyobj_t *
  GetPolyobj(int polyNum);
static int
  GetPolyobjMirror(int poly);
static void
  ThrustMobj(mobj_t *mobj, seg_t *seg, polyobj_t *po);
static void
  UpdateSegBBox(seg_t *seg);
static void
  RotatePt(int an, fixed_t *x, fixed_t *y, fixed_t startSpotX, fixed_t startSpotY);
static void
  UnLinkPolyobj(polyobj_t *po);
static void
  LinkPolyobj(polyobj_t *po);
static boolean
  CheckMobjBlocking(seg_t *seg, polyobj_t *po);
static void
  InitBlockMap();
static void
  IterFindPolySegs(int x, int y, seg_t **segList);
static void
  SpawnPolyobj(int index, int tag, boolean crush);
static void
               TranslateToStartSpot(int tag, int originX, int originY);
  ...
  PO_Init(int lump)
{
  ...
  for (i = 0; i < numthings; i++, mt++)
  {
    ...
    { // Polyobj StartSpot Pt.
      ...
      SpawnPolyobj(polyIndex, spawnthing.angle, (spawnthing.type == PO_SPAWNCRUSH_TYPE));
      polyIndex++;
    }
  }
  mt = (mapthing_t *)data;
