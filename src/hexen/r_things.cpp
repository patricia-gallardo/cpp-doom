...
void
  R_InstallSpriteLump(int lump, unsigned frame, unsigned rotation, boolean flipped)
{
  int r;
  ...
  if (vis->mobjflags & (MF_SHADOW | MF_ALTSHADOW))
  {
    if (vis->mobjflags & MF_TRANSLATION)
    {
      ...
      dc_translation = translationtables - 256
                       + vis->clazz * ((maxplayers - 1) * 256) + ((vis->mobjflags & MF_TRANSLATION) >> (MF_TRANSSHIFT - 8));
    }
...
//
  ...
  if (viewplayer->powers[pw_invulnerability] && viewplayer->clazz == PCLASS_CLERIC)
  {
  ...
  R_DrawMasked()
{
  ...
  if (viewangleoffset <= 1024 << ANGLETOFINESHIFT || viewangleoffset >= -(1024 << ANGLETOFINESHIFT))
