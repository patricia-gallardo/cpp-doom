...
static mobj_t *
  RoughBlockCheck(mobj_t *mo, int index);
  ...
  while (link)
  {
    ...
    else if (mo->type == MT_MINOTAUR) // looking around minotaur
    {
      ...
      if ((link->flags & MF_COUNTKILL) || (link->player && (link != master)))
      {
        ...
        if ((link->type == MT_MINOTAUR) && (link->special1.m == mo->special1.m))
        {
          link = link->bnext;
          continue;
        }
    ...
    else if (mo->type == MT_MSTAFF_FX2) // bloodscourge
    {
          ...
          && !(link->flags2 & MF2_DORMANT))
      {
        ...
        {
          master = mo->target;
          angle  = R_PointToAngle2(master->x, master->y, link->x, link->y) - master->angle;
          angle >>= 24;
