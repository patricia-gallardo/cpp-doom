...
static boolean
     CheckedLockedDoor(mobj_t *mo, byte lock);
...
boolean
  EV_SectorSoundChange(byte *args)
{
  int     secNum;
  boolean rtn;
...
boolean
  EV_LineSearchForPuzzleItem(line_t *line, byte *args, mobj_t *mo)
{
  player_t  *player;
  int        i;
  int        type;
  ...
  P_ExecuteLineSpecial(int special, byte *args, line_t *line, int side, mobj_t *mo)
{
  ...
  switch (special)
  {
    ...
    case 138: // Floor_Waggle
      buttonSuccess = EV_StartFloorWaggle(args[0], args[1], args[2], args[3], args[4]);
      break;
...
boolean
  P_ActivateLine(line_t *line, mobj_t *mo, int side, int activationType)
{
  byte    args[5];
  int     lineActivation;
  boolean repeat;
