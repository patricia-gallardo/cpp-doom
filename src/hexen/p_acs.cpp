...
static void
  StartOpenACS(int number, int infoIndex, int offset);
static void
  ScriptFinished(int number);
static boolean
  TagBusy(int tag);
static boolean
  AddToACSStore(int map, int number, byte *args);
static int
  GetACSIndex(int number);
static void
  Push(int value);
static int
  Pop();
...
static void
  ACSAssert(int condition, const char *fmt, ...)
{
  char    buf[128];
  va_list args;
...
static char *
  StringLookup(int string_index)
{
  ACSAssert(string_index >= 0,
            "negative string index: %d < 0",
            string_index);
  ACSAssert(string_index < ACStringCount,
            "invalid string index: %d >= %d",
            string_index,
            ACStringCount);
  ...
  ReadOffset()
{
  ...
  ACSAssert(offset < ActionCodeSize, "invalid lump offset: %d >= %d", offset, ActionCodeSize);
  return offset;
}
  ...
  P_LoadACScripts(int lump)
{
  ...
  M_snprintf(EvalContext, sizeof(EvalContext), "header parsing of lump #%d", lump);
  ...
  for (i = 0; i < ACStringCount; ++i)
  {
    ...
    ACSAssert(memchr(ACStrings[i], '\0', ActionCodeSize - offset) != NULL,
              "string %d missing terminating NUL",
              i);
  }
  ...
  P_StartACS(int number, int map, byte *args, mobj_t *activator, line_t *line, int side)
{
  ...
  { // Script not found
    ...
    M_snprintf(ErrorMsg, sizeof(ErrorMsg), "P_STARTACS ERROR: UNKNOWN SCRIPT %d", number);
  ...
  T_InterpretACS(acs_t *script)
{
  ...
  do
  {
    M_snprintf(EvalContext, sizeof(EvalContext), "script %d @0x%x", ACSInfo[script->infoIndex].number, PCodeOffset);
    cmd = ReadCodeInt();
    M_snprintf(EvalContext, sizeof(EvalContext), "script %d @0x%x, cmd=%d", ACSInfo[script->infoIndex].number, PCodeOffset, cmd);
  ...
  CmdLSpec1()
{
  ...
  special     = ReadCodeInt();
  SpecArgs[0] = Pop();
  P_ExecuteLineSpecial(special, SpecArgs, ACScript->line, ACScript->side, ACScript->activator);
  return SCRIPT_CONTINUE;
}
