...
boolean
  P_CheckMeleeRange2(mobj_t *actor)
{
  mobj_t *mo;
  fixed_t dist;
...
boolean
  P_LookForMonsters(mobj_t *actor)
{
  int        count;
  mobj_t    *mo;
  thinker_t *think;
...
void
  A_SetInvulnerable(mobj_t *actor)
{
...
void
  A_UnSetInvulnerable(mobj_t *actor)
{
  actor->flags2 &= ~MF2_INVULNERABLE;
}
...
void
  A_UnSetReflective(mobj_t *actor)
{
  actor->flags2 &= ~MF2_REFLECTIVE;
...
boolean
  P_UpdateMorphedMonster(mobj_t *actor, int tics)
{
  mobj_t    *fog;
  fixed_t    x;
  fixed_t    y;
...
void
  FaceMovementDirection(mobj_t *actor)
{
  ...
  A_MinotaurChase(mobj_t *actor)
{
  ...
  if (!actor->target || (actor->target->health <= 0) || !(actor->target->flags & MF_SHOOTABLE))
  { // look for a new target
...
void
  P_InitCreatureCorpseQueue(boolean corpseScan)
{
  thinker_t *think;
  mobj_t    *mo;
...
void
  A_SerpentRaiseHump(mobj_t *actor)
{
...
void
  A_SerpentLowerHump(mobj_t *actor)
{
  actor->floorclip += 4 * FRACUNIT;
}
...
void
  A_SerpentHumpDecide(mobj_t *actor)
{
...
void
  A_SerpentBirthScream(mobj_t *actor)
{
...
void
  A_SerpentDiveSound(mobj_t *actor)
{
  S_StartSound(actor, SFX_SERPENT_ACTIVE);
}
...
void
  A_SerpentCheckForAttack(mobj_t *actor)
{
  if (!actor->target)
  {
    return;
...
void
  A_SerpentChooseAttack(mobj_t *actor)
{
...
void
  A_SerpentMeleeAttack(mobj_t *actor)
{
  if (!actor->target)
  {
    return;
...
void
  A_SerpentMissileAttack(mobj_t *actor)
{
  if (!actor->target)
  {
    return;
...
void
  A_SerpentSpawnGibs(mobj_t *actor)
{
  mobj_t *mo;
  int     r1, r2;
...
void
  A_SerpentHeadCheck(mobj_t *actor)
{
  if (actor->z <= actor->floorz)
  {
...
void
  A_CentaurDropStuff(mobj_t *actor)
{
  mobj_t *mo;
  angle_t angle;
...
void
  A_BishopMissileWeave(mobj_t *actor)
{
  fixed_t newX, newY;
  int     weaveXY, weaveZ;
  int     angle;
...
void
  A_BishopMissileSeek(mobj_t *actor)
{
...
void
  A_BishopSpawnBlur(mobj_t *actor)
{
  mobj_t *mo;
  ...
  A_BishopPainBlur(mobj_t *actor)
{
  ...
  mo = P_SpawnMobj(actor->x + (r3 << 12), actor->y + (r2 << 12), actor->z + (r1 << 11), MT_BISHOPPAINBLUR);
  if (mo)
  {
    mo->angle = actor->angle;
  }
...
static void
  DragonSeek(mobj_t *actor, angle_t thresh, angle_t turnMax)
{
  int     dir;
  int     dist;
  angle_t delta;
  ...
  { // attack the destination mobj if it's attackable
    ...
    if (actor->angle - R_PointToAngle2(actor->x, actor->y, target->x, target->y) < ANG45 / 2)
    {
...
void
  A_DragonInitFlight(mobj_t *actor)
{
  int search;
...
void
  A_DragonCheckCrash(mobj_t *actor)
{
  if (actor->z <= actor->floorz)
  {
...
void
  A_WraithRaiseInit(mobj_t *actor)
{
  actor->flags2 &= ~MF2_DONTDRAW;
...
void
  A_IceGuyMissilePuff(mobj_t *actor)
{
...
void
  A_IceGuyMissileExplode(mobj_t *actor)
{
  mobj_t      *mo;
  unsigned int i;
...
void
  A_SorcBallOrbit(mobj_t *actor);
void
  A_SorcSpinBalls(mobj_t *actor);
void
  A_SpeedBalls(mobj_t *actor);
void
  A_SlowBalls(mobj_t *actor);
void
  A_StopBalls(mobj_t *actor);
void
  A_AccelBalls(mobj_t *actor);
void
  A_DecelBalls(mobj_t *actor);
void
  A_SorcBossAttack(mobj_t *actor);
void
  A_SpawnFizzle(mobj_t *actor);
void
  A_CastSorcererSpell(mobj_t *actor);
void
  A_SorcUpdateBallAngle(mobj_t *actor);
void
  A_BounceCheck(mobj_t *actor);
void
  A_SorcFX1Seek(mobj_t *actor);
void
  A_SorcOffense1(mobj_t *actor);
void
  A_SorcOffense2(mobj_t *actor);
  ...
  A_SorcBallOrbit(mobj_t *actor)
{
  ...
  switch (mode)
  {
    ...
    case SORC_STOPPING: // Balls stopping
      if ((parent->special2.i == actor->type) && (parent->args[1] > SORCBALL_SPEED_ROTATIONS) && ((angle - (parent->angle >> ANGLETOFINESHIFT)) < (30 << 5)))
      {
        // Can stop now
...
void
  A_SmokePuffExit(mobj_t *actor)
{
  P_SpawnMobj(actor->x, actor->y, actor->z, MT_MNTRSMOKEEXIT);
}

void
  A_SorcererBishopEntry(mobj_t *actor)
{
...
void
  A_ClassBossHealth(mobj_t *actor)
{
...
void
  A_IceCheckHeadDone(mobj_t *actor)
{
  if (actor->special2.i == 666)
  {
...
void
  A_FreezeDeathChunks(mobj_t *actor)
{
  int     i;
  int     r1, r2, r3;
  mobj_t *mo;
...
void
  KoraxFire1(mobj_t *actor, int type);
void
  KoraxFire2(mobj_t *actor, int type);
void
  KoraxFire3(mobj_t *actor, int type);
void
  KoraxFire4(mobj_t *actor, int type);
void
  KoraxFire5(mobj_t *actor, int type);
void
  KoraxFire6(mobj_t *actor, int type);
void
  KSpiritInit(mobj_t *spirit, mobj_t *korax);
...
void
  A_KoraxChase(mobj_t *actor)
{
  mobj_t *spot;
  int     lastfound;
  byte    args[3] = { 0, 0, 0 };

  if ((!actor->special2.i) && (actor->health <= (actor->info->spawnhealth / 2)))
  {
    lastfound = 0;
...
void
  A_KoraxStep(mobj_t *actor)
{
  A_Chase(actor);
}

void
  A_KoraxStep2(mobj_t *actor)
{
...
void
  A_KSpiritSeeker(mobj_t *actor, angle_t thresh, angle_t turnMax)
{
  int     dir;
  int     dist;
  angle_t delta;
