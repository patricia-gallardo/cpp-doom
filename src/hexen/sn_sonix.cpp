...
static void
  VerifySequencePtr(int *base, int *ptr);
static int
                 GetSoundOffset(char *name);
...
void
  SN_StartSequence(mobj_t *mobj, int sequence)
{
  seqnode_t *node;
...
void
  SN_StartSequenceName(mobj_t *mobj, char *name)
{
  int i;
  ...
  for (node = SequenceListHead; node; node = node->next)
  {
    ...
    switch (*node->sequencePtr)
    {
      ...
      case SS_CMD_DELAYRAND:
        node->delayTics = *(node->sequencePtr + 1) + M_Random() % (*(node->sequencePtr + 2) - *(node->sequencePtr + 1));
        node->sequencePtr += 2;
        node->currentSoundID = 0;
        break;
      case SS_CMD_VOLUME:
...
int
  SN_GetSequenceOffset(int sequence, int *sequencePtr)
{
...
void
  SN_ChangeNodeData(int nodeNum, int seqOffset, int delayTics, int volume, int currentSoundID)
{
  int        i;
  seqnode_t *node;
