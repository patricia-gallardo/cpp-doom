...
static void
  SetDormantArtifact(mobj_t *arti);
static void
  TryPickupArtifact(player_t *player, artitype_t artifactType, mobj_t *artifact);
static void
  TryPickupWeapon(player_t *player, pclass_t weaponClass, weapontype_t weaponType, mobj_t *weapon, const char *message);
static void
  TryPickupWeaponPiece(player_t *player, pclass_t matchClass, int pieceValue, mobj_t *pieceMobj);
...
void
  P_HideSpecialThing(mobj_t *thing)
{
  ...
  TryPickupWeaponPiece(player_t *player, pclass_t matchClass, int pieceValue, mobj_t *pieceMobj)
{
  ...
  {
    P_ExecuteLineSpecial(pieceMobj->special, pieceMobj->args, NULL, 0, player->mo);
    pieceMobj->special = 0;
  }
  if (remove)
  {
...
int
  P_GiveKey(player_t *player, keytype_t key)
{
...
void
  A_RestoreSpecialThing2(mobj_t *thing)
{
  thing->flags |= MF_SPECIAL;
...
mobj_t *
  ActiveMinotaur(player_t *master)
{
  mobj_t               *mo;
  player_t             *plr;
  thinker_t            *think;
  ...
  P_KillMobj(mobj_t *source, mobj_t *target)
{
  ...
  { // Normal death
    if ((target->type == MT_FIREDEMON) && (target->z <= target->floorz + 2 * FRACUNIT) && (target->info->xdeathstate))
    {
...
void
  P_MinotaurSlam(mobj_t *source, mobj_t *target)
{
  angle_t angle;
  fixed_t thrust;
...
void
  P_AutoUseHealth(player_t *player, int saveHealth)
{
  int i;
  int count;
  int normalCount;
  ...
  P_DamageMobj(mobj_t *target, mobj_t *inflictor, mobj_t *source, int damage)
{
  ...
  if (target->player)
  {
    if (damage < 1000 && ((target->player->cheats & CF_GODMODE) || target->player->powers[pw_invulnerability]))
    {
      return;
    }
  }
  ...
  if (inflictor)
  {
    switch (inflictor->type)
    {
      ...
      case MT_TELOTHER_FX5:
        if ((target->flags & MF_COUNTKILL) && (target->type != MT_SERPENT) && (target->type != MT_SERPENTLEADER) && (!(target->flags2 & MF2_BOSS)))
        {
  ...
  if (player)
  {
    savedPercent = AutoArmorSave[player->clazz]
                   + player->armorpoints[ARMOR_ARMOR] + player->armorpoints[ARMOR_SHIELD] + player->armorpoints[ARMOR_HELMET] + player->armorpoints[ARMOR_AMULET];
    if (savedPercent)
  ...
  { // Death
    ...
    if (source && (source->player) && (source->player->readyweapon == WP_FOURTH))
    {
      ...
      && !(target->flags & MF_SKULLFLY))
  {
    if (inflictor && (inflictor->type >= MT_LIGHTNING_FLOOR && inflictor->type <= MT_LIGHTNING_ZAP))
    {
      ...
      { // "electrocute" the target
        ...
        {
          if ((target->type == MT_CENTAUR) || (target->type == MT_CENTAURLEADER) || (target->type == MT_ETTIN))
          {
