...
constexpr bool
  data_or_hook_empty(data_or_hook hook)
{
  return hook.index() == 0;
}
constexpr bool
  data_or_hook_has_value(data_or_hook hook)
{
  return hook.index() != 0;
}
...
void
  RD_ClearMapWindow();
void
  RD_DisplayLine(int x1, int y1, int x2, int y2, float gray);
void
  RD_DrawNodeLine(node_t *node);
void
  RD_DrawLineCheck(seg_t *line);
void
  RD_DrawLine(seg_t *line);
void
                     RD_DrawBBox(fixed_t *bbox);
...
void
  R_DrawMaskedColumn(column_t *column, signed int baseclip);
...
void
  R_DrawMasked();
void
                     R_ClipVisSprite(vissprite_t *vis, int xl, int xh);
