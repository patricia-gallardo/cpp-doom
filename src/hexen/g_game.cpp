        ...
        switch (players[i].cmd.buttons & BT_SPECIALMASK)
        {
          ...
          case BTS_SAVEGAME:
            if (!savedescription[0])
            {
              ...
              {
                M_StringCopy(savedescription, "SAVE GAME", sizeof(savedescription));
              }
            }
            savegameslot =
...
void
  G_PlayerExitMap(int playerNumber)
{
  int       i;
  player_t *player;
  int       flightPower;
...
void
  G_TeleportNewMap(int map, int position)
{
...
void
  G_Completed(int map, int position)
{
...
void
  G_DeferredNewGame(skill_t skill)
{
  TempSkill  = skill;
  gameaction = ga_newgame;
}
...
void
  G_RecordDemo(skill_t skill, int numplayers, int episode, int map, char *name)
{
  int i;
  int maxsize;
