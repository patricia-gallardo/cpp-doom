...
void
  CT_queueChatChar(char ch);
void
  CT_ClearChatMessage(int player);
void
  CT_AddChar(int player, char c);
void
            CT_BackSpace(int player);
  ...
  for (i = 0; i < maxplayers; i++)
  {
    ...
    if ((c = players[i].cmd.chatchar) != 0)
    {
      ...
      else if (c == KEY_ENTER)
      {
        ...
        if (numplayers > 2)
        {
          M_StringCopy(plr_lastmsg[i], CT_FromPlrText[i], sizeof(plr_lastmsg[i]));
        ...
        if (i != consoleplayer && (chat_dest[i] == consoleplayer + 1 || chat_dest[i] == CT_PLR_ALL)
            && *chat_msg[i])
        {
