...
static void
              PlayerLandedOnThing(mobj_t *mo, mobj_t *onmobj);
...
void
  P_FloorBounceMissile(mobj_t *mo)
{
    ...
    { // Blocked move
      ...
      else if (mo->flags & MF_MISSILE)
      {
        if (mo->flags2 & MF2_FLOORBOUNCE)
        {
          if (BlockingMobj)
          {
            if ((BlockingMobj->flags2 & MF2_REFLECTIVE) || ((!BlockingMobj->player) && (!(BlockingMobj->flags & MF_COUNTKILL))))
            {
              fixed_t speed;
...
void
  P_MonsterFallingDamage(mobj_t *mo)
{
  int damage;
  int mom;
  ...
  P_MobjThinker(mobj_t *mobj)
{
  ...
  */
  ...
  { // Floating item bobbing motion (special1 is height)
    mobj->z = mobj->floorz + mobj->special1.i + FloatBobOffsets[(mobj->health++) & 63];
  }
