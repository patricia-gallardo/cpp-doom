  ...
  A_PotteryExplode(mobj_t *actor)
{
  ...
  { // Spawn an item
    ...
    { // Only spawn monsters if not -nomonsters
      P_SpawnMobj(actor->x, actor->y, actor->z, TranslateThingType[actor->args[0]]);
    }
  }
  P_RemoveMobj(actor);
}
...
void
  A_PotteryChooseBit(mobj_t *actor)
{
...
void
  A_CorpseBloodDrip(mobj_t *actor)
{
  if (P_Random() > 128)
  {
    return;
...
void
  A_PoisonBagDamage(mobj_t *actor)
{
  int         bobIndex;
...
boolean
  A_LocalQuake(byte *args, mobj_t *actor)
{
  mobj_t *focus, *target;
  int     lastfound = 0;
  int     success   = false;
...
void
  A_ThrustBlock(mobj_t *actor)
{
  actor->flags |= MF_SOLID;
}

void
  A_ThrustImpale(mobj_t *actor)
{
...
void
  A_BatSpawnInit(mobj_t *actor)
{
  actor->special1.i = 0; // Frequency count
}

void
  A_BatSpawn(mobj_t *actor)
{
  mobj_t *mo;
  int     delta;
  angle_t angle;
