...
void
  P_PlayerNextArtifact(player_t *player);
...
void
  P_MorphPlayerThink(player_t *player)
{
  mobj_t *pmo;
...
boolean
  P_UndoPlayerMorph(player_t *player)
{
  mobj_t      *fog;
  mobj_t      *mo;
  mobj_t      *pmo;
...
void
  P_ArtiTeleportOther(player_t *player)
{
  mobj_t *mo;
...
void
  P_TeleportToPlayerStarts(mobj_t *victim)
{
  int     i, selections = 0;
  fixed_t destX, destY;
  angle_t destAngle;
...
void
  P_TeleportToDeathmatchStarts(mobj_t *victim)
{
  int     i, selections;
  fixed_t destX, destY;
  angle_t destAngle;
...
void
  P_BlastMobj(mobj_t *source, mobj_t *victim, fixed_t strength)
{
  angle_t angle, ang;
  mobj_t *mo;
  fixed_t x, y, z;
  ...
  P_BlastRadius(player_t *player)
{
  ...
  for (think = thinkercap.next; think != &thinkercap; think = think->next)
  {
    ...
    else if (!(mo->flags & MF_COUNTKILL) && !(mo->player) && !(mo->flags & MF_MISSILE))
  ...
  for (think = thinkercap.next; think != &thinkercap; think = think->next)
  {
    ...
    switch (player->clazz)
    {
      case PCLASS_FIGHTER: // Radius armor boost
        if ((P_GiveArmor(mo->player, ARMOR_ARMOR, 1)) || (P_GiveArmor(mo->player, ARMOR_SHIELD, 1)) || (P_GiveArmor(mo->player, ARMOR_HELMET, 1)) || (P_GiveArmor(mo->player, ARMOR_AMULET, 1)))
        {
          effective = true;
      ...
      case PCLASS_MAGE: // Radius mana boost
        amount = 50 + (P_Random() % 50);
        if ((P_GiveMana(mo->player, MANA_1, amount)) || (P_GiveMana(mo->player, MANA_2, amount)))
        {
          effective = true;
          S_StartSound(mo, SFX_MYSTICINCANT);
        }
  ...
  P_UseArtifact(player_t *player, artitype_t arti)
{
  ...
  switch (arti)
  {
    ...
    case arti_poisonbag:
      ...
      {
        mo = P_SpawnMobj(player->mo->x, player->mo->y, player->mo->z - player->mo->floorclip + 35 * FRACUNIT, MT_THROWINGBOMB);
        if (mo)
        {
          mo->angle =
