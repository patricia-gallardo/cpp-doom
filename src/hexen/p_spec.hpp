...
// when needed
boolean
  P_ExecuteLineSpecial(int special, byte *args, line_t *line, int side, mobj_t *mo);
boolean
  P_ActivateLine(line_t *ld, mobj_t *mo, int side, int activationType);
...
void
  P_PlayerOnSpecialFlat(player_t *player, int floorType);
...
int
  P_FindSectorFromTag(int tag, int start);
// int P_FindMinSurroundingLight(sector_t *sector,int max);
sector_t *
  getNextSector(line_t *line, sector_t *sec);
line_t *
  P_FindLine(int lineTag, int *searchPosition);
...
void
  T_Light(light_t *light);
void
  P_SpawnPhasedLight(sector_t *sector, int base, int index);
void
  P_SpawnLightSequence(sector_t *sector, int indexStep);
boolean
  EV_SpawnLight(line_t *line, byte *arg, lighttype_t type);
...
int
  EV_DoPlat(line_t *line, byte *args, plattype_e type, int amount);
void
  P_AddActivePlat(plat_t *plat);
void
  P_RemoveActivePlat(plat_t *plat);
void
  EV_StopPlat(line_t *line, byte *args);
...
int
  EV_DoDoor(line_t *line, byte *args, vldoor_e type);
void
...
int
  EV_DoCeiling(line_t *line, byte *args, ceiling_e type);
void
...
int
  EV_CeilingCrushStop(line_t *line, byte *args);
...
int
  EV_BuildStairs(line_t *line, byte *args, int direction, stairs_e type);
int
...
boolean
  P_Teleport(mobj_t *thing, fixed_t x, fixed_t y, angle_t angle, boolean useFog);
boolean
  EV_Teleport(int tid, mobj_t *thing, boolean fog);
...
void
  P_LoadACScripts(int lump);
boolean
  P_StartACS(int number, int map, byte *args, mobj_t *activator, line_t *line, int side);
boolean
  P_StartLockedACS(line_t *line, byte *args, mobj_t *mo, int side);
boolean
  P_TerminateACS(int number, int map);
boolean
  P_SuspendACS(int number, int map);
void
  T_InterpretACS(acs_t *script);
void
  P_TagFinished(int tag);
void
  P_PolyobjFinished(int po);
void
  P_ACSInitNewGame();
void
  P_CheckACSStore();
void
                  CheckACSPresent(int number);
...
boolean
  EV_ThingProjectile(byte *args, boolean gravity);
boolean
  EV_ThingSpawn(byte *args, boolean fog);
boolean
  EV_ThingActivate(int tid);
boolean
  EV_ThingDeactivate(int tid);
boolean
  EV_ThingRemove(int tid);
boolean
  EV_ThingDestroy(int tid);
