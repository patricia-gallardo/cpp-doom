...
void
  MN_DrTextAYellow(const char *text, int x, int y)
{
  char     c;
  patch_t *p;
  ...
  DrawMainMenu()
{
  ...
  V_DrawPatch(37, 80, cache_lump_num<patch_t *>(MauloBaseLump + (frame + 2) % 7, PU_CACHE));
  ...
  DrawClassMenu()
{
  ...
  V_DrawPatch(174 + 24, 8 + 12, cache_lump_num<patch_t *>(W_GetNumForName(walkLumpName[clazz]) + ((MenuTime >> 3) & 3), PU_CACHE));
}
...
static boolean
  ReadDescriptionForSlot(int slot, char *description)
{
  FILE   *fp;
  boolean found;
  char    name[100];
  ...
  DrawSlider(Menu_t *menu, int item, int width, int slot)
{
  ...
  V_DrawPatch(x + 4 + slot * 8, y + 7, cache_lump_name<patch_t *>("M_SLDKB", PU_CACHE));
}
