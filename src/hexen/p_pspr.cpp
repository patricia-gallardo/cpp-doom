...
void
  P_SetPspriteNF(player_t *player, int position, statenum_t stnum)
{
  pspdef_t *psp;
  state_t  *state;
...
void
  P_ActivateMorphWeapon(player_t *player)
{
...
void
  P_PostMorphWeapon(player_t *player, weapontype_t weapon)
{
  player->pendingweapon          = WP_NOCHANGE;
  player->readyweapon            = weapon;
  player->psprites[ps_weapon].sy = WEAPONBOTTOM;
  P_SetPsprite(player, ps_weapon, WeaponInfo[weapon][player->clazz].upstate);
}
  ...
  P_BringUpWeapon(player_t *player)
{
  ...
  if (player->clazz == PCLASS_FIGHTER && player->pendingweapon == WP_SECOND
      && player->mana[MANA_1])
  {
    stnum = S_FAXEUP_G;
  }
...
boolean
  P_CheckMana(player_t *player)
{
  manatype_t mana;
  int        count;

  mana  = WeaponInfo[player->readyweapon][player->clazz].mana;
  count = WeaponManaUse[player->clazz][player->readyweapon];
  if (mana == MANA_BOTH)
  {
  ...
  do
  {
    if (player->weaponowned[WP_THIRD]
        && player->mana[MANA_2] >= WeaponManaUse[player->clazz][WP_THIRD])
    {
      player->pendingweapon = WP_THIRD;
    }
    else if (player->weaponowned[WP_SECOND]
             && player->mana[MANA_1] >= WeaponManaUse[player->clazz][WP_SECOND])
    {
      player->pendingweapon = WP_SECOND;
    }
    else if (player->weaponowned[WP_FOURTH]
             && player->mana[MANA_1] >= WeaponManaUse[player->clazz][WP_FOURTH]
             && player->mana[MANA_2] >= WeaponManaUse[player->clazz][WP_FOURTH])
    {
  ...
  P_SetPsprite(player, ps_weapon, WeaponInfo[player->readyweapon][player->clazz].downstate);
  return (false);
}
  ...
  P_FireWeapon(player_t *player)
{
  ...
  P_SetMobjState(player->mo, static_cast<statenum_t>(PStateAttack[player->clazz])); // S_PLAY_ATK1);
  ...
  A_WeaponReady(player_t *player, pspdef_t *psp)
{
  ...
  if (player->mo->state >= &states[PStateAttack[player->clazz]]
      && player->mo->state <= &states[PStateAttackEnd[player->clazz]])
  {
  ...
  A_Raise(player_t *player, pspdef_t *psp)
{
  ...
  {
    P_SetPsprite(player, ps_weapon, WeaponInfo[player->readyweapon][player->clazz].readystate);
  }
}
...
void
  A_SnoutAttack(player_t *player, pspdef_t *psp)
{
  angle_t angle;
  int     damage;
  int     slope;
...
void
  A_FHammerAttack(player_t *player, pspdef_t *psp)
{
  angle_t angle;
...
hammerdone:
  if (player->mana[MANA_2] < WeaponManaUse[player->clazz][player->readyweapon])
...
void
  A_FHammerThrow(player_t *player, pspdef_t *psp)
{
  mobj_t *mo;
...
void
  A_FSwordAttack(player_t *player, pspdef_t *psp)
{
  mobj_t *pmo;
  ...
  A_FSwordFlames(mobj_t *actor)
{
  ...
  for (i = 1 + (P_Random() & 3); i; i--)
  {
    ...
    r2 = P_Random();
    r3 = P_Random();
    P_SpawnMobj(actor->x + ((r3 - 128) << 12), actor->y + ((r2 - 128) << 12), actor->z + ((r1 - 128) << 11), MT_FSWORD_FLAME);
  }
}
...
void
  A_MWandAttack(player_t *player, pspdef_t *psp)
{
  mobj_t *mo;
...
void
  A_LightningReady(player_t *player, pspdef_t *psp)
{
...
void
  A_MLightningAttack2(mobj_t *actor)
{
  mobj_t *fmo, *cmo;
...
void
  A_MLightningAttack(player_t *player, pspdef_t *psp)
{
...
void
  A_LightningRemove(mobj_t *actor)
{
  mobj_t *mo;
...
//============================================================================
void
  MStaffSpawn(mobj_t *pmo, angle_t angle)
{
  mobj_t *mo;
...
void
  A_MStaffAttack(player_t *player, pspdef_t *psp)
{
  angle_t angle;
  mobj_t *pmo;
...
void
  A_MStaffPalette(player_t *player, pspdef_t *psp)
{
  int pal;
...
void
  MStaffSpawn2(mobj_t *actor, angle_t angle)
{
  mobj_t *mo;
...
void
  A_FPunchAttack(player_t *player, pspdef_t *psp)
{
  angle_t angle;
  int     damage;
  int     slope;
...
void
  A_FAxeAttack(player_t *player, pspdef_t *psp)
{
  angle_t angle;
  mobj_t *pmo = player->mo;
  fixed_t power;
...
axedone:
  ...
  {
    player->mana[MANA_1] -=
      WeaponManaUse[player->clazz][player->readyweapon];
    if (player->mana[MANA_1] <= 0)
    {
...
void
  A_CMaceAttack(player_t *player, pspdef_t *psp)
{
  angle_t angle;
  int     damage;
  int     slope;
...
void
  A_CStaffAttack(player_t *player, pspdef_t *psp)
{
  mobj_t *mo;
  mobj_t *pmo;
...
void
  A_CStaffMissileSlither(mobj_t *actor)
{
  fixed_t newX, newY;
  int     weaveXY;
  int     angle;
...
void
  A_CStaffInitBlink(player_t *player, pspdef_t *psp)
{
...
void
  A_CStaffCheckBlink(player_t *player, pspdef_t *psp)
{
  if (!--player->mo->special1.i)
  {
...
void
  A_CFlameAttack(player_t *player, pspdef_t *psp)
{
  mobj_t *mo;
...
void
  A_CHolyAttack(player_t *player, pspdef_t *psp)
{
  player->mana[MANA_1] -= WeaponManaUse[player->clazz][player->readyweapon];
  player->mana[MANA_2] -= WeaponManaUse[player->clazz][player->readyweapon];
...
void
  A_CHolyPalette(player_t *player, pspdef_t *psp)
{
  int pal;
...
static void
  CHolySeekerMissile(mobj_t *actor, angle_t thresh, angle_t turnMax)
{
  int     dir;
  int     dist;
  angle_t delta;
...
static void
  CHolyTailFollow(mobj_t *actor, fixed_t dist)
{
  mobj_t *child;
  int     an;
  ...
  if (child)
  {
    ...
    if (P_TryMove(child, actor->x + FixedMul(dist, finecosine[an]), actor->y + FixedMul(dist, finesine[an])))
    {
  ...
  A_CHolyTail(mobj_t *actor)
{
  ...
  if (parent)
  {
    ...
    else if (P_TryMove(actor, parent->x - FixedMul(14 * FRACUNIT, finecosine[parent->angle >> ANGLETOFINESHIFT]), parent->y - FixedMul(14 * FRACUNIT, finesine[parent->angle >> ANGLETOFINESHIFT])))
    {
...
void
  A_CHolyCheckScream(mobj_t *actor)
{
  A_CHolySeek(actor);
  if (P_Random() < 20)
  {
...
void
  A_FireConePL1(player_t *player, pspdef_t *psp)
{
  angle_t angle;
  int     damage;
  int     i;
