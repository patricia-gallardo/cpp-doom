...
static char *
  GetRegistryString(registry_value_t *reg_val)
{
  HKEY  key;
  DWORD len;
  DWORD valtype;
...
static boolean
  DirIsFile(const char *path, const char *filename)
{
...
static char *
  CheckDirectoryHasIWAD(const char *dir, const char *iwadname)
{
  char *filename;
  char *probe;
...
static char *
  SearchDirectoryForIWAD(const char *dir, int mask, GameMission_t *mission)
{
  char  *filename;
  size_t i;
...
static GameMission_t
  IdentifyIWADByName(const char *name, int mask)
{
  size_t        i;
...
static void
  AddIWADPath(const char *path, const char *suffix)
{
...
char *
  D_FindWADByName(const char *name)
{
  char *path;
  char *probe;
  int   i;
...
char *
  D_TryFindWADByName(const char *filename)
{
  char *result;
