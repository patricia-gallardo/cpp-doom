...
// not all systems.
static boolean
  IsDirectory(char *dir, struct dirent *de)
{
...
static void
  FreeStringList(char **globs, int num_globs)
{
  int i;
...
glob_t *
  I_StartMultiGlob(const char *directory, int flags, const char *glob, ...)
{
  char  **globs;
  int     num_globs;
  glob_t *result;
...
glob_t *
  I_StartGlob(const char *directory, const char *glob, int flags)
{
  return I_StartMultiGlob(directory, flags, glob, NULL);
}

void
  I_EndGlob(glob_t *glob)
{
  if (glob == NULL)
  {
    return;
...
static boolean
  MatchesGlob(const char *name, const char *glob, int flags)
{
  int n, g;
...
static boolean
  MatchesAnyGlob(const char *name, glob_t *glob)
{
  int i;
...
static void
  SortFilenames(char **filenames, int len, int flags)
{
  char *pivot, *tmp;
  int   i, left_len, cmp;
