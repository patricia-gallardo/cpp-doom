...
static int
  VarInGroup(int *variable, int **group)
{
  unsigned int i;
...
static void
  CheckKeyGroup(int *variable, int **group)
{
  unsigned int i;
...
static void
  KeySetCallback(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(variable))
{
...
static void
  AddKeyControl(TXT_UNCAST_ARG(table), const char *name, int *var)
{
...
static void
  AddSectionLabel(TXT_UNCAST_ARG(table), const char *title, boolean add_space)
{
  TXT_CAST_ARG(txt_table_t, table);
  char buf[64];
...
static void
  ConfigExtraKeys(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
  txt_window_t     *window;
...
static void
  OtherKeysDialog(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
  txt_window_t     *window;
  txt_table_t      *table;
  txt_scrollpane_t *scrollpane;
...
void
  ConfigKeyboard(TXT_UNCAST_ARG(widget), void *user_data)
{
  txt_window_t   *window;
