...
static int
  MousePressCallback(txt_window_t *window,
                     int           x,
                     int           y,
                     int           b,
...
static void
  OpenPromptWindow(txt_mouse_input_t *mouse_input)
{
  txt_window_t *window;
...
static void
  TXT_MouseInputSizeCalc(TXT_UNCAST_ARG(mouse_input))
{
  TXT_CAST_ARG(txt_mouse_input_t, mouse_input);
...
static void
  GetMouseButtonDescription(int button, char *buf, size_t buf_len)
{
  switch (button)
  {
    case 0:
...
static void
  TXT_MouseInputDrawer(TXT_UNCAST_ARG(mouse_input))
{
  TXT_CAST_ARG(txt_mouse_input_t, mouse_input);
  char buf[20];
  int  i;
...
static void
  TXT_MouseInputDestructor(TXT_UNCAST_ARG(mouse_input))
{
}

static int
  TXT_MouseInputKeyPress(TXT_UNCAST_ARG(mouse_input), int key)
{
  TXT_CAST_ARG(txt_mouse_input_t, mouse_input);
...
static void
  TXT_MouseInputMousePress(TXT_UNCAST_ARG(widget), int x, int y, int b)
{
...
txt_mouse_input_t *
  TXT_NewMouseInput(int *variable)
{
