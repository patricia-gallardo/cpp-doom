...
static int
  PhysicalForVirtualButton(int vbutton)
{
...
static int
  VirtualButtonForVariable(int *variable)
{
  int i;
...
// button.
static void
  ClearVariablesUsingButton(int physbutton)
{
  int vbutton;
  int i;
...
static int
  EventCallback(SDL_Event *event, TXT_UNCAST_ARG(joystick_input))
{
...
static void
  PromptWindowClosed(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(joystick))
{
...
static void
  OpenPromptWindow(txt_joystick_input_t *joystick_input)
{
  txt_window_t *window;
...
static void
  TXT_JoystickInputSizeCalc(TXT_UNCAST_ARG(joystick_input))
{
  TXT_CAST_ARG(txt_joystick_input_t, joystick_input);
...
static void
  GetJoystickButtonDescription(int vbutton, char *buf, size_t buf_len)
{
  M_snprintf(buf, buf_len, "BUTTON #%i", PhysicalForVirtualButton(vbutton) + 1);
}

static void
  TXT_JoystickInputDrawer(TXT_UNCAST_ARG(joystick_input))
{
  TXT_CAST_ARG(txt_joystick_input_t, joystick_input);
  char buf[20];
  int  i;
...
static void
  TXT_JoystickInputDestructor(TXT_UNCAST_ARG(joystick_input))
{
}

static int
  TXT_JoystickInputKeyPress(TXT_UNCAST_ARG(joystick_input), int key)
{
  TXT_CAST_ARG(txt_joystick_input_t, joystick_input);
...
static void
  TXT_JoystickInputMousePress(TXT_UNCAST_ARG(widget),
                              int x,
                              int y,
                              int b)
{
...
txt_joystick_input_t *
  TXT_NewJoystickInput(int *variable)
{
