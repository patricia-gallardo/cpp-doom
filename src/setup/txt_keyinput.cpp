...
static int
  KeyPressCallback(txt_window_t *window, int key, TXT_UNCAST_ARG(key_input))
{
...
static void
  ReleaseGrab(TXT_UNCAST_ARG(window), TXT_UNCAST_ARG(unused))
{
  // SDL2-TODO: Needed?
...
static void
  OpenPromptWindow(txt_key_input_t *key_input)
{
  txt_window_t *window;
...
static void
  TXT_KeyInputSizeCalc(TXT_UNCAST_ARG(key_input))
{
  TXT_CAST_ARG(txt_key_input_t, key_input);
...
static void
  TXT_KeyInputDrawer(TXT_UNCAST_ARG(key_input))
{
  TXT_CAST_ARG(txt_key_input_t, key_input);
  char buf[20];
  int  i;
...
static void
  TXT_KeyInputDestructor(TXT_UNCAST_ARG(key_input))
{
}

static int
  TXT_KeyInputKeyPress(TXT_UNCAST_ARG(key_input), int key)
{
  TXT_CAST_ARG(txt_key_input_t, key_input);
...
static void
  TXT_KeyInputMousePress(TXT_UNCAST_ARG(widget), int x, int y, int b)
{
