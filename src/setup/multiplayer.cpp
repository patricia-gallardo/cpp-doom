...
static boolean
  IsChexQuest(const iwad_t *iwad)
{
  return !strcmp(iwad->name, "chex.wad");
}

static void
  AddWADs(execute_context_t *exec)
{
  int have_wads = 0;
  int i;
...
static void
  AddExtraParameters(execute_context_t *exec)
{
  int i;
...
static void
  AddIWADParameter(execute_context_t *exec)
{
  if (iwadfile != NULL)
  {
...
static void
  StartServerGame(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
  StartGame(1);
}

static void
  StartSinglePlayerGame(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
  StartGame(0);
}
...
static void
  SetExMyWarp(TXT_UNCAST_ARG(widget), void *val)
{
  int l;
...
static void
  SetMAPxyWarp(TXT_UNCAST_ARG(widget), void *val)
{
  int l;
...
static void
  CloseLevelSelectDialog(TXT_UNCAST_ARG(button), TXT_UNCAST_ARG(window))
{
...
static void
  LevelSelectDialog(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(user_data))
{
  txt_window_t *window;
  txt_button_t *button;
  const iwad_t *iwad;
...
static void
  IWADSelected(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
  const iwad_t *iwad;
...
static void
  UpdateWarpType(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
...
static txt_window_action_t *
  StartGameAction(int multiplayer)
{
...
static void
  OpenWadsWindow(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(user_data))
{
  txt_window_t *window;
  int           i;
...
static void
  OpenExtraParamsWindow(TXT_UNCAST_ARG(widget),
...
static void
  StartGameMenu(const char *window_title, int multiplayer)
{
  txt_window_t *window;
...
void
  StartMultiGame(TXT_UNCAST_ARG(widget), void *user_data)
{
  StartGameMenu("Start multiplayer game", 1);
}

void
  WarpMenu(TXT_UNCAST_ARG(widget), void *user_data)
{
  StartGameMenu("Level Warp", 0);
}

static void
  DoJoinGame(void *unused1, void *unused2)
{
  execute_context_t *exec;
...
static void
  SelectQueryAddress(TXT_UNCAST_ARG(button),
...
static void
  QueryResponseCallback(net_addr_t      *addr,
...
static void
  QueryPeriodicCallback(TXT_UNCAST_ARG(results_table))
{
  TXT_CAST_ARG(txt_table_t, results_table);
...
static void
  QueryWindowClosed(TXT_UNCAST_ARG(window), void *unused)
{
  TXT_SetPeriodicCallback(NULL, NULL, 0);
}

static void
  ServerQueryWindow(const char *title)
{
...
static void
  FindInternetServer(TXT_UNCAST_ARG(widget),
                     TXT_UNCAST_ARG(user_data))
{
...
static void
  FindLANServer(TXT_UNCAST_ARG(widget),
                TXT_UNCAST_ARG(user_data))
{
  NET_StartLANQuery();
...
void
  JoinMultiGame(TXT_UNCAST_ARG(widget), void *user_data)
{
  txt_window_t   *window;
...
void
  MultiplayerConfig(TXT_UNCAST_ARG(widget), void *user_data)
{
  txt_window_t *window;
  txt_label_t  *label;
  txt_table_t  *table;
