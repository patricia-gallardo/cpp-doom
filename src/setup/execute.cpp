...
void
  PassThroughArguments(execute_context_t *context)
{
  int i;
...
void
  AddCmdLineParameter(execute_context_t *context, const char *s, ...)
{
  va_list args;
...
static unsigned int
  WaitForProcessExit(HANDLE subprocess)
{
  DWORD exit_code;
...
static void
  ConcatWCString(wchar_t *buf, const char *value)
{
  MultiByteToWideChar(CP_OEMCP, 0, value, strlen(value) + 1, buf + wcslen(buf), strlen(value) + 1);
}
...
static wchar_t *
  BuildCommandLine(const char *program, const char *arg)
{
...
static int
  ExecuteCommand(const char *program, const char *arg)
{
...
static char *
  GetFullExePath(const char *program)
{
  char *result;
  char *sep;
  size_t result_len;
...
int
  ExecuteDoom(execute_context_t *context)
{
...
static void
  TestCallback(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(data))
{
