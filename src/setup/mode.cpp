...
static void
  SetExecutable(mission_config_t *config)
{
  char *extension;
...
static void
  SetMission(mission_config_t *config)
{
...
static mission_config_t *
  GetMissionForName(char *name)
{
  int i;
...
static boolean
  CheckExecutableName(GameSelectCallback callback)
{
...
static void
  GameSelected(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(config))
{
...
static void
  OpenGameSelectDialog(GameSelectCallback callback)
{
  ...
  for (i = 0; i < arrlen(mission_configs); ++i)
  {
    ...
    if (iwads[0] != NULL)
    {
      ...
      TXT_AddWidget(window, TXT_NewButton2(mission_configs[i].label, GameSelected, &mission_configs[i]));
      ++num_games;
    }
