...
using joystick_config_t = struct
{
  const char *name; // Config file name
  int         value;
};

using known_joystick_t = struct
{
  const char              *name;
...
static const known_joystick_t *
  GetJoystickType(int index)
{
...
static void
  LoadConfigurationSet(const joystick_config_t *configs)
{
...
static boolean
  SetJoystickGUID(SDL_JoystickID joy_id)
{
...
static int
  CalibrationEventCallback(SDL_Event *event, void *user_data)
{
...
static void
  CalibrateWindowClosed(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
...
static void
  CalibrateJoystick(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(unused))
{
  ...
  TXT_SetWindowAction(calibration_window, TXT_HORIZ_CENTER, TXT_NewWindowAbortAction(calibration_window));
...
static void
  AddJoystickControl(TXT_UNCAST_ARG(table), const char *label, int *var)
{
...
void
  ConfigJoystick(TXT_UNCAST_ARG(widget), void *user_data)
{
  txt_window_t *window;
