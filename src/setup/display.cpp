...
static void
  WindowSizeSelected(TXT_UNCAST_ARG(widget), TXT_UNCAST_ARG(size))
{
...
static txt_radiobutton_t *
  SizeSelectButton(window_size_t *size)
{
  char               buf[15];
...
static void
  GenerateSizesTable(TXT_UNCAST_ARG(widget),
...
static void
  AdvancedDisplayConfig(TXT_UNCAST_ARG(widget),
                        TXT_UNCAST_ARG(sizes_table))
{
  TXT_CAST_ARG(txt_table_t, sizes_table);
  txt_window_t   *window;
...
void
  ConfigDisplay(TXT_UNCAST_ARG(widget), void *user_data)
{
  txt_window_t        *window;
