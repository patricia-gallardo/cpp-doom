...
static void
  ParseVorbisComments(file_metadata_t *metadata, FILE *fs)
{
  uint32_t     buf;
...
static void
  ParseFlacStreaminfo(file_metadata_t *metadata, FILE *fs)
{
  byte buf[34];
...
static void
  ParseFlacFile(file_metadata_t *metadata, FILE *fs)
{
  byte         header[4];
...
static void
  ParseOggIdHeader(file_metadata_t *metadata, FILE *fs)
{
  byte buf[21];
...
static void
  ParseOggFile(file_metadata_t *metadata, FILE *fs)
{
  byte         buf[7];
  unsigned int offset;
...
static void
  ReadLoopPoints(const char *filename, file_metadata_t *metadata)
{
  FILE *fs;
  char  header[4];
...
static char *
  GetFullPath(const char *musicdir, const char *path)
{
  char *result;
...
// GetFullPath().
static char *
  ExpandFileExtension(const char *musicdir, const char *filename)
{
...
static void
  AddSubstituteMusic(const char *musicdir, const char *hash_prefix, const char *filename)
{
  subst_music_t *s;
  char          *path;
...
static const char *
  ParseSubstituteLine(char *musicdir, char *line)
{
  const char *hash_prefix;
  char       *filename;
  char       *p;
...
static boolean
  ReadSubstituteConfig(char *musicdir, const char *filename)
{
  char *buffer;
  char *line;
  int   linenum = 1;
...
static void
  I_MP_SetMusicVolume(int volume)
{
...
static void
  I_MP_PlaySong(void *handle, boolean looping)
{
  int loops;
...
static void
  I_MP_UnRegisterSong(void *handle)
{
...
static void *
  I_MP_RegisterSong(void *data, int len)
{
  const char *filename;
  Mix_Music  *music;
