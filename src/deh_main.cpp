...
static deh_section_t *
  GetSectionByName(char *name)
{
  unsigned int i;
...
static boolean
  CheckSignatures(deh_context_t *context)
{
  size_t i;
  char  *line;
...
static void
  DEH_ParseContext(deh_context_t *context)
{
  ...
  DEH_AutoLoadPatches(const char *path)
{
  ...
  glob = I_StartMultiGlob(path, GLOB_FLAG_NOCASE | GLOB_FLAG_SORTED, "*.deh", "*.bex", "*.hhe", "*.seh", NULL); // [crispy] *.bex
  for (;;)
  {
