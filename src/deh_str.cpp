...
using deh_substitution_t = struct
{
  char *from_text;
  char *to_text;
};
...
static deh_substitution_t *
  SubstitutionForString(const char *s)
{
  int entry;
...
boolean
  DEH_HasStringReplacement(const char *s)
{
...
static void
  DEH_AddToHashtable(deh_substitution_t *sub);
...
void
  DEH_AddStringReplacement(const char *from_text, const char *to_text)
{
...
static format_arg_t
  NextFormatArgument(const char **str)
{
...
static boolean
  ValidArgumentReplacement(format_arg_t original,
...
static boolean
  ValidFormatReplacement(const char *original, const char *replacement)
{
  const char *rover1;
  const char *rover2;
...
static const char *
  FormatStringReplacement(const char *s)
{
  const char *repl;
...
void
  DEH_fprintf(FILE *fstream, const char *fmt, ...)
{
  va_list     args;
  const char *repl;
...
void
  DEH_snprintf(char *buffer, size_t len, const char *fmt, ...)
{
  va_list     args;
  const char *repl;
