...
static void
  MapFile(posix_wad_file_t *wad, const char *filename)
{
  void *result;
  int   protection;
  int   flags;
...
static wad_file_t *
  W_POSIX_OpenFile(const char *path)
{
...
static void
  W_POSIX_CloseFile(wad_file_t *wad)
{
...
size_t
  W_POSIX_Read(wad_file_t *wad, unsigned int offset, void *buffer, size_t buffer_len)
{
  posix_wad_file_t *posix_wad;
  byte             *byte_buffer;
  size_t            bytes_read;
